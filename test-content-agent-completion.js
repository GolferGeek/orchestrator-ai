...
async function createContentTask() {
  ...
  try {
    const taskRequest = {
      method: 'process',
      prompt: 'Create a compelling blog post about the benefits of AI-powered project management tools for remote teams. Include SEO optimization, engaging headlines, and a strong call-to-action. Target mid-market companies and focus on productivity improvements.',
      timeoutSeconds: 300,
    };

    const response = await fetch(`${API_BASE_URL}/agents/specialists/content/tasks`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
...
async function runTest() {
  console.log('🧪 Starting Content Agent Completion Test');
  ...
  // Step 2: Create Content task
  const taskData = await createContentTask();
  ...
  const hasContentKeywords = finalTask && finalTask.response && 
    (finalTask.response.includes('AI') || finalTask.response.includes('project management') || finalTask.response.includes('remote teams'));
  ...
  console.log(`${hasContentKeywords ? '✅' : '❌'} Contains Expected Content: ${hasContentKeywords ? 'YES' : 'NO'}`);
  
  if (taskCompleted && hasResponse && hasContentKeywords) {
    console.log('\n🎉 SUCCESS! Content agent is working correctly!');
    ...
    if (!hasContentKeywords) console.log('   - Response does not contain expected content keywords');
    process.exit(1);
  }
}
