...
async function createChatSupportTask() {
  console.log('\n💬 Creating chat support task...');
  
  try {
    const taskRequest = {
      method: 'executeTask',
      prompt: 'Hi! I\'m new to Orchestrator AI and would like to understand what your platform can do for my business. Can you explain the key features and benefits? I\'m particularly interested in how it can help with customer support automation.',
    ...
    const response = await fetch(`${API_BASE_URL}/agents/sales/chat_support/tasks`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
...
async function runTest() {
  console.log('🧪 Starting Chat Support Agent Completion Test');
  ...
  // Step 2: Create chat support task
  const taskData = await createChatSupportTask();
  ...
  if (taskCompleted && hasResponse) {
    console.log('\n🎉 SUCCESS! Chat Support agent is working correctly!');
    console.log('\n💬 Generated Support Response:');
    console.log('=' .repeat(50));
    console.log(finalTask.response?.substring(0, 600) + (finalTask.response?.length > 600 ? '...' : ''));
    process.exit(0);
  } else {
