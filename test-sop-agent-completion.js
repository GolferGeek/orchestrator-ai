...
async function createSOPTask() {
  console.log('\n📋 Creating SOP task...');
  
  try {
    const taskRequest = {
      method: 'executeTask',
      prompt: 'Create a comprehensive Standard Operating Procedure for handling customer support tickets. Include escalation procedures, response time requirements, and quality control measures.',
    ...
    const response = await fetch(`${API_BASE_URL}/agents/operations/sop/tasks`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
...
async function runTest() {
  console.log('🧪 Starting SOP Agent Completion Test');
  ...
  // Step 2: Create SOP task
  const taskData = await createSOPTask();
  ...
  if (taskCompleted && hasResponse) {
    console.log('\n🎉 SUCCESS! SOP agent is working correctly!');
