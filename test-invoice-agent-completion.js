...
async function createInvoiceTask() {
  console.log('\n📋 Creating invoice task...');
  
  try {
    const taskRequest = {
      method: 'executeTask',
      prompt: 'Create an invoice for TechCorp consulting services. Include 40 hours of strategic consulting at $175/hour and project documentation for $2,500 fixed fee.',
    ...
    const response = await fetch(`${API_BASE_URL}/agents/finance/invoice/tasks`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
...
async function checkEvaluations() {
  ...
  try {
    ...
    // Look for our recent invoice task
    const recentEvaluation = evaluations.find(eval => 
      eval.metadata?.agentName?.includes('Invoice') ||
      eval.content?.includes('TechCorp') ||
      eval.content?.includes('INV-') ||
      Date.now() - new Date(eval.timestamp).getTime() < 300000 // Last 5 minutes
    );
    
    if (recentEvaluation) {
      console.log('✅ Found recent invoice evaluation:');
      ...
      console.log('⚠️  No recent invoice evaluation found');
      return null;
    }
  } catch (error) {
...
async function runTest() {
  console.log('🧪 Starting Invoice Agent Completion Test');
  ...
  // Step 2: Create invoice task
  const taskData = await createInvoiceTask();
