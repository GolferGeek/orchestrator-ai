...
async function createCalendarTask() {
  console.log('\n🗓️ Creating calendar task...');
  
  try {
    const taskRequest = {
      method: 'executeTask',
      prompt: 'Schedule a quarterly board meeting with 8 executives across different time zones (EST, PST, GMT, CET). Need optimal time slots considering executive availability, international coordination, and backup options for conflicts.',
    ...
    const response = await fetch(`${API_BASE_URL}/agents/operations/calendar/tasks`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
...
async function runTest() {
  console.log('🧪 Starting Calendar Agent Completion Test');
  ...
  // Step 2: Create calendar task
  const taskData = await createCalendarTask();
  ...
  if (taskCompleted && hasResponse) {
    console.log('\n🎉 SUCCESS! Calendar agent is working correctly!');
    console.log('\n🗓️ Generated Calendar Content:');
    console.log('=' .repeat(50));
