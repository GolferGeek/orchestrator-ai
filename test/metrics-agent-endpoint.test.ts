...
describe('Metrics Agent Endpoint Test', () => {
  ...
  it('should handle another business query through the complete workflow', async () => {
    console.log('\n🚀 Testing Second Business Query');
    console.log('==================================================');

    const testQuery = 'What is the current platform activity? Show me user counts, task counts, and recent activity.';
    ...
    const response = await request(app.getHttpServer())
      ...
      .send({
        message: testQuery,
        context: 'platform analytics and user engagement analysis'
      })
      .expect(200);
    ...
    const responseText = JSON.stringify(response.body);
    expect(responseText.toLowerCase()).toMatch(/user|task|platform|activity|count/);

    console.log('✅ Platform analytics query completed successfully');
    console.log('✅ MCP workflow executed for business intelligence');
