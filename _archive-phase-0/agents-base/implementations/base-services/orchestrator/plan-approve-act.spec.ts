...
describe('Plan-Approve-Act Lifecycle - Real LLM Tests', () => {
  ...
  describe('Complete Project Lifecycle', () => {
    ...
    it('should execute complete marketing campaign project lifecycle', async () => {
      ...
      const planRequest: OrchestratorInput = {
        prompt:
          'Create a comprehensive marketing campaign for launching our new AI-powered project management tool. Target B2B software companies, include market research, content creation, competitive analysis, and coordinated launch activities over 4 weeks.',
        userId: 'test-user-lifecycle',
        conversationId: 'test-conv-lifecycle',
        conversationHistory: [],
      };

      const planResponse = await marketingManager.executeTask(
        'executeTask',
        planRequest,
      );
      ...
      console.log(
        `📊 Plan Response: ${(planResponse.message || planResponse.response)?.substring(0, 200)}...`,
      );
      ...
      const approvalRequest: OrchestratorInput = {
        prompt:
          'I approve the marketing campaign plan. Please proceed with execution starting with the first step.',
        userId: 'test-user-lifecycle',
        conversationId: 'test-conv-lifecycle',
        ...
        conversationHistory: [
          ...
          },
          {
            role: 'assistant',
            content:
              planResponse.message || planResponse.response || 'Plan created',
            timestamp: new Date().toISOString(),
            metadata: {
      ...
      const approvalResponse = await marketingManager.executeTask(
        'executeTask',
        approvalRequest,
      );
      ...
      console.log(
        `📊 Next Action: ${(approvalResponse.message || approvalResponse.response)?.substring(0, 200)}...`,
      );
      ...
      const executionRequest: OrchestratorInput = {
        prompt:
          'Execute the first step of the approved marketing campaign plan. Begin with market research.',
        userId: 'test-user-lifecycle',
        conversationId: 'test-conv-lifecycle',
        projectId: planResponse.projectId,
        conversationHistory: [
      ...
      const executionResponse = await marketingManager.executeTask(
        'executeTask',
        executionRequest,
      );
      ...
      if (executionResponse.metadata?.delegatedAgent) {
        console.log(
          `🎯 Delegated to: ${executionResponse.metadata.delegatedAgent}`,
        );
      }
      console.log(
        `📊 Execution Result: ${(executionResponse.message || executionResponse.response)?.substring(0, 200)}...`,
      );

      // VALIDATION: Check that we have a complete project workflow
      expect([
        planResponse.projectId,
        approvalResponse.projectId,
        executionResponse.projectId,
      ]).toEqual([
        planResponse.projectId,
        planResponse.projectId,
        planResponse.projectId,
    ...
    it('should handle iterative plan refinement with real LLM feedback', async () => {
      ...
      const initialPlanResponse = await marketingManager.executeTask(
        'executeTask',
        initialPlanRequest,
      );
      ...
      const refinementRequest: OrchestratorInput = {
        prompt:
          "Actually, let's expand this plan to include social media promotion, email marketing campaigns, and influencer outreach to amplify the blog content. Also add competitive analysis to ensure our messaging differentiates from competitors.",
        userId: 'test-user-iterative',
        conversationId: 'test-conv-iterative',
      ...
      const refinementResponse = await marketingManager.executeTask(
        'executeTask',
        refinementRequest,
      );
      ...
      console.log(
        `📊 Refinement includes: ${(refinementResponse.message || refinementResponse.response)?.substring(0, 300)}...`,
      );

      // STEP 3: Validate refinement intelligence
      const responseText = (
        refinementResponse.message ||
      ...
      expect(responseText).toMatch(
        /(social|media|email|influencer|competitive|analysis)/,
      );
    ...
    it('should handle conditional approval workflows with LLM intelligence', async () => {
      ...
      const planRequest: OrchestratorInput = {
        prompt:
          'Plan a major product rebranding campaign that will require significant budget, multiple departments, and executive oversight. This is a high-stakes initiative.',
      ...
      const modificationRequest: OrchestratorInput = {
        prompt:
          'Before I approve this plan, I need to reduce the budget and timeline. Can you modify the plan to be more conservative while still achieving the core rebranding objectives?',
        userId: 'test-user-approval',
        conversationId: 'test-conv-approval',
        projectId: planResponse.projectId,
        conversationHistory: [
      ...
      const modificationResponse = await marketingManager.executeTask(
        'executeTask',
        modificationRequest,
      );
      ...
      const finalApprovalRequest: OrchestratorInput = {
        prompt:
          'Perfect! This revised plan looks much more manageable. I approve this modified rebranding plan. Please proceed with the first phase.',
        userId: 'test-user-approval',
        conversationId: 'test-conv-approval',
        projectId: planResponse.projectId,
        conversationHistory: [
      ...
      const finalApprovalResponse = await marketingManager.executeTask(
        'executeTask',
      ...
      console.log(
        `🧠 LLM handled: Plan → Modification Request → Revised Plan → Approval → Execution`,
      );
    ...
    it('should execute multi-step marketing project with real agent delegation', async () => {
      ...
      const projectRequest: OrchestratorInput = {
        prompt:
          'Create and execute a content marketing project: (1) research competitor content strategies, (2) create three blog posts based on the research, (3) develop social media promotion for the blog posts. I want to see this executed step by step.',
        userId: 'test-user-multistep',
        conversationId: 'test-conv-multistep',
        conversationHistory: [],
      };

      const projectResponse = await marketingManager.executeTask(
        'executeTask',
        projectRequest,
      );
      ...
      if (
        projectResponse.action === 'DELEGATE' &&
        projectResponse.metadata?.delegatedAgent
      ) {
        console.log(
          `🎯 First Step Delegated to: ${projectResponse.metadata.delegatedAgent}`,
        );

        // Validate that it chose appropriate agent for competitor research
        expect(
          ['competitors', 'market_research'].some((agent) =>
            projectResponse.metadata?.delegatedAgent
              ?.toLowerCase()
              .includes(agent),
          ),
      ...
      const nextStepRequest: OrchestratorInput = {
        prompt:
          'Great! The competitor research is complete. Now please proceed with step 2: creating the three blog posts based on the research findings.',
        userId: 'test-user-multistep',
        conversationId: 'test-conv-multistep',
        ...
        conversationHistory: [
          ...
          },
          {
            ...
            metadata: {
              agentName:
                projectResponse.metadata?.delegatedAgent || 'orchestrator',
              projectId: projectResponse.projectId,
            },
          },
        ],
      ...
      const nextStepResponse = await marketingManager.executeTask(
        'executeTask',
        nextStepRequest,
      );
      ...
      if (nextStepResponse.metadata?.delegatedAgent) {
        console.log(
          `🎯 Step 2 Delegated to: ${nextStepResponse.metadata.delegatedAgent}`,
        );

        // Should delegate blog creation to blog-focused agent
        expect(
          ['blog_post', 'content'].some((agent) =>
            nextStepResponse.metadata?.delegatedAgent
              ?.toLowerCase()
              .includes(agent),
          ),
        ).toBe(true);
        ...
        console.log(
          `✅ Intelligent Agent Selection for Content Creation Phase`,
        );
      }

      console.log('\n🎉 Multi-Step Execution Test Successful!');
      console.log(
        `🧠 LLM demonstrated intelligent multi-step project management`,
      );
  ...
  describe('Error Recovery and Resilience', () => {
    ...
    it('should handle step failures with intelligent recovery', async () => {
      ...
      const projectRequest: OrchestratorInput = {
        prompt:
          'Create a marketing campaign, but I want to see how you handle if something goes wrong during execution.',
      ...
      const errorScenarioRequest: OrchestratorInput = {
        prompt:
          'There was an error with the previous step - the marketing research agent is unavailable. How should we proceed with the campaign?',
        userId: 'test-user-recovery',
        conversationId: 'test-conv-recovery',
        projectId: projectResponse.projectId,
        conversationHistory: [
      ...
      const recoveryResponse = await marketingManager.executeTask(
        'executeTask',
        errorScenarioRequest,
      );

      expect(recoveryResponse.success).toBe(true);
      console.log(`✅ Error Recovery Response Generated`);
      console.log(
        `🔄 Recovery Strategy: ${(recoveryResponse.message || recoveryResponse.response)?.substring(0, 200)}...`,
      );
      ...
      expect(responseText).toMatch(
        /(alternative|instead|other|skip|continue|next)/,
      );
