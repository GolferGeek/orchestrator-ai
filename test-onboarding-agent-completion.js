...
async function createOnboardingTask() {
  ...
  try {
    const taskRequest = {
      method: 'executeTask',
      prompt: 'Create a comprehensive onboarding checklist for a new software engineer starting next Monday. Include pre-boarding, first day, and first week activities.',
    ...
    const response = await fetch(`${API_BASE_URL}/agents/hr/onboarding/tasks`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
...
async function checkEvaluations() {
  ...
  try {
    ...
    // Look for our recent onboarding task
    const recentEvaluation = evaluations.find(eval => 
      eval.metadata?.agentName?.includes('Onboarding') ||
      eval.content?.includes('software engineer') ||
      eval.content?.includes('checklist') ||
      eval.content?.includes('onboarding') ||
      Date.now() - new Date(eval.timestamp).getTime() < 300000 // Last 5 minutes
    );
    
    if (recentEvaluation) {
      console.log('✅ Found recent onboarding evaluation:');
      ...
      console.log('⚠️  No recent onboarding evaluation found');
      return null;
    }
  } catch (error) {
...
async function runTest() {
  console.log('🧪 Starting Onboarding Agent Completion Test');
  ...
  const taskData = await createOnboardingTask();
  ...
  if (taskCompleted && hasResponse && inEvaluations && evaluationComplete) {
    console.log('\n🎉 SUCCESS! Onboarding agent is working correctly!');
