...
async function createMarketingSwarmTask() {
  console.log('\n📈 Creating marketing swarm task...');
  
  try {
    const taskRequest = {
      method: 'executeTask',
      prompt: 'Create a comprehensive marketing campaign for our new AI-powered project management software. I need blog posts, social media content, and email marketing materials that highlight productivity benefits and team collaboration features.',
    ...
    const response = await fetch(`${API_BASE_URL}/agents/marketing/marketing_swarm/tasks`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
...
async function checkEvaluations() {
  ...
  try {
    ...
    // Look for our recent marketing swarm task
    const recentEvaluation = evaluations.find(eval => 
      eval.metadata?.agentName?.includes('Marketing') ||
      eval.metadata?.agentName?.includes('Swarm') ||
      eval.content?.includes('marketing campaign') ||
      eval.content?.includes('AI-powered project') ||
      Date.now() - new Date(eval.timestamp).getTime() < 300000 // Last 5 minutes
    );
    
    if (recentEvaluation) {
      console.log('✅ Found recent marketing swarm evaluation:');
      ...
      console.log('⚠️  No recent marketing swarm evaluation found');
      return null;
    }
  } catch (error) {
...
async function waitForTaskCompletion(taskId, maxWaitTime = 120000) { // Extended to 2 minutes for complex task
  console.log(`\n⏳ Waiting for task ${taskId} to complete...`);
  
  const startTime = Date.now();
  const pollInterval = 3000; // Check every 3 seconds for slower multi-agent task
...
async function runTest() {
  console.log('🧪 Starting Marketing Swarm Agent Completion Test');
  ...
  // Step 2: Create marketing swarm task
  const taskData = await createMarketingSwarmTask();
  ...
  await new Promise(resolve => setTimeout(resolve, 5000)); // Wait 5s for evaluation processing
  ...
  if (taskCompleted && hasResponse && hasResponseMetadata) {
    console.log('\n🎉 SUCCESS! Marketing Swarm Agent is completing tasks correctly!');
    console.log(`📊 Response length: ${finalTask.response?.length || 0} characters`);
    console.log(`🔧 Metadata keys: ${Object.keys(finalTask.responseMetadata || {}).length}`);
    process.exit(0);
  } else {
