...
PROJECT_ROOT_FOR_DOTENV = _CONFIG_FILE_PATH.parents[5]
...
_calculated_project_root_for_pydantic = Path(__file__).resolve().parents[5]
_dotenv_path_for_pydantic_config = _calculated_project_root_for_pydantic / ".env"

DEFAULT_MARKDOWN_CONTEXT_PATH = _calculated_project_root_for_pydantic / "apps" / "api" / "python" / "fastapi_v1" / "markdown_context"
...
# print(f"[CONFIG_PY_DEBUG] Settings() instantiated. TEST_API_SECRET_KEY from settings object: '{settings.TEST_API_SECRET_KEY}'") # DEBUG REMOVED
# print(f"[CONFIG_PY_DEBUG] SUPABASE_URL from settings object: '{settings.SUPABASE_URL}'") # DEBUG REMOVED
# print(f"[CONFIG_PY_DEBUG] SUPABASE_ANON_KEY from settings object (showing first 5 chars only): '{str(settings.SUPABASE_ANON_KEY)[:5] if settings.SUPABASE_ANON_KEY else None}'") # DEBUG REMOVED
