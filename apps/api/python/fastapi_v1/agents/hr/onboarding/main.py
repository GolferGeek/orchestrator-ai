...
AGENT_ID: str = "onboarding-agent-v1"
AGENT_NAME: str = "onboarding" 
AGENT_DESCRIPTION: str = "Manages employee onboarding processes."
AGENT_VERSION: str = "0.1.0" 
MCP_TARGET_AGENT_ID: str = AGENT_NAME
CONTEXT_FILE_NAME: str = "onboarding_agent.md"
PRIMARY_CAPABILITY_NAME: str = "query_onboarding_via_mcp"
PRIMARY_CAPABILITY_DESCRIPTION: str = "Manages employee onboarding processes by relaying queries to an MCP."

# Configure logging
logger = logging.getLogger(__name__)

class OnboardingAgentService(MCPContextAgentBaseService):
    """
    Onboarding Agent Service that queries an MCP for context-aware responses.
...
agent_router = APIRouter(
    prefix=f"/agents/{OnboardingAgentService.department_name}/{OnboardingAgentService.agent_name.lower().replace(' ', '_')}",
    tags=[f"{OnboardingAgentService.agent_name}"] 
)

async def get_onboarding_agent_service(
...
async def get_agent_card_route(
    service: OnboardingAgentService = Depends(get_onboarding_agent_service)
):
...
async def process_tasks_route(
    ...
    logger.info(f"Onboarding Agent /tasks endpoint received task request ID: {task_request.id} with input: {log_input_text}")

    try:
        task_response = await service.handle_task_send(params=task_request)
        if task_response:
            logger.info(f"Onboarding Agent /tasks endpoint successfully processed task {task_request.id}. Returning Task object. Final state: {task_response.status.state}")
            return task_response
        else:
            logger.error(f"Onboarding Agent /tasks endpoint: handle_task_send returned None for task {task_request.id}. This is unexpected.")
        ...
        logger.error(f"Onboarding Agent /tasks endpoint: JSONRPCError for task {task_request.id}: {rpc_error.message}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Task processing error: {rpc_error.message}")
    except HTTPException: 
        raise
    except Exception as e:
        logger.error(f"Onboarding Agent /tasks endpoint: Unexpected error for task {task_request.id}: {e}", exc_info=True)
...
async def get_task_status_route(
    ...
    logger.info(f"Onboarding Agent /tasks/{{task_id}} GET endpoint received request for task ID: {task_id}")
    task = await service.handle_task_get(task_id=task_id)
    if not task:
        logger.warning(f"Task {task_id} not found for GET request to Onboarding Agent.")
        raise HTTPException(status_code=404, detail=f"Task with ID {task_id} not found for Onboarding Agent.")
    logger.info(f"Onboarding Agent returning task {task_id} with status {task.status.state}")
    return task
