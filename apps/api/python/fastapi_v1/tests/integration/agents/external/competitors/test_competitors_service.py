...
from apps.api.python.fastapi_v1.agents.external.competitors.main import AGENT_VERSION as COMPETITORS_VERSION
from apps.api.python.fastapi_v1.a2a_protocol.types import Message, TextPart, TaskSendParams
...
async def test_competitors_process_message_success(
    ...
    with pytest.MonkeyPatch.context() as mp:
        ...
        mp.setattr("apps.api.python.fastapi_v1.shared.mcp.mcp_client.MCPClient.query_agent_aggregate", mock_mcp.query_agent_aggregate)
...
@pytest.mark.asyncio
async def test_competitors_process_message_mcp_connection_error(
    client_and_app: tuple[httpx.AsyncClient, FastAPI],
    mock_openai_service: AsyncMock
):
    client, _ = client_and_app
    ...
    # Construct payload using TaskSendParams
    request_payload = TaskSendParams(
        id="test-comp-unexpected-error",
        message=message
    )
    
    # Mock MCPClient to raise MCP connection error
    with pytest.MonkeyPatch.context() as mp:
        mock_mcp = AsyncMock()
        mock_mcp.query_agent_aggregate.side_effect = MCPConnectionError("Failed to connect")
        mp.setattr("apps.api.python.fastapi_v1.shared.mcp.mcp_client.MCPClient.query_agent_aggregate", mock_mcp.query_agent_aggregate)
