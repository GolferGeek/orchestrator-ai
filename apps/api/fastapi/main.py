...
from apps.api.fastapi.a2a_protocol.task_store import TaskStoreService
...
from apps.api.fastapi.llm.openai_service import OpenAIService
from apps.api.fastapi.core.config import settings
from apps.api.fastapi.core.db import get_supabase_client, get_current_supabase_client, get_anon_supabase_client, get_current_supabase_service_client
from supabase import Client as SupabaseClient
from apps.api.fastapi.auth.dependencies import get_current_authenticated_user, get_supabase_client_as_current_user, oauth2_scheme
from apps.api.fastapi.auth.schemas import SupabaseAuthUser
from apps.api.fastapi.sessions.schemas import SessionCreate, SessionResponse, SessionListResponse

# Import the MCP router
from apps.api.fastapi.shared.mcp.mcp_routes import mcp_router
from apps.api.fastapi.auth.routes import router as auth_router
from apps.api.fastapi.sessions.routes import router as sessions_router
...
def load_agent_services(app_to_configure: FastAPI):
    ...
    for agent_category_dir in agents_base_dir.iterdir():
        if agent_category_dir.is_dir() and (agent_category_dir / "__init__.py").exists():
            ...
            # Construct module path relative to apps.api.fastapi (which is effectively the current top-level package for execution via main.py)
            if agent_category_dir.name == "orchestrator":
                module_path_for_import = f"apps.api.fastapi.agents.{agent_category_dir.name}.main"
            ...
            for agent_dir in agent_category_dir.iterdir():
                if agent_dir.is_dir() and (agent_dir / "__init__.py").exists() and (agent_dir / "main.py").exists():
                    module_path_for_import = f"apps.api.fastapi.agents.{agent_category_dir.name}.{agent_dir.name}.main"
