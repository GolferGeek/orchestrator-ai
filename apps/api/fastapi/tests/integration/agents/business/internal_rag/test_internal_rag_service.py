...
from httpx import AsyncClient # For client type hint

# Import agent-specific constants and the service class from the Internal RAG agent's main module
from apps.api.fastapi.agents.business.internal_rag.main import (
    InternalRagAgentService, # Corrected: Was InternalRagService
...
API_V1_ROOT = Path(__file__).resolve().parents[5] # Corrected: up to apps/api/
INTERNAL_RAG_CONTEXT_FILE_PATH = API_V1_ROOT / "markdown_context" / INTERNAL_RAG_CONTEXT_FILE

@pytest.mark.asyncio
async def test_get_internal_rag_agent_card(client_and_app: tuple[AsyncClient, FastAPI]):
    """Test the get_agent_card method of InternalRagAgentService via endpoint."""
    client, _ = client_and_app
    response = await client.get(f"/agents/{InternalRagAgentService.department_name}/{INTERNAL_RAG_AGENT_NAME}/agent-card")
...
@pytest.mark.asyncio
@patch("apps.api.python.fastapi_v1.agents.business.internal_rag.main.MCPClient.query_agent_aggregate", new_callable=AsyncMock)
async def test_internal_rag_task_send_success_valid_query(client_and_app: tuple[AsyncClient, FastAPI], mocker: AsyncMock):
    ...
    mock_query_aggregate = mocker.patch(
        "apps.api.python.fastapi_v1.agents.business.internal_rag.main.MCPClient.query_agent_aggregate",
    ...
    response = await client.post(
        f"/agents/{InternalRagAgentService.department_name}/{INTERNAL_RAG_AGENT_NAME}/tasks", 
...
@pytest.mark.asyncio
@patch("apps.api.python.fastapi_v1.agents.business.internal_rag.main.MCPClient.query_agent_aggregate", new_callable=AsyncMock)
async def test_internal_rag_task_send_success_no_results_query(client_and_app: tuple[AsyncClient, FastAPI], mocker: AsyncMock):
    client, _ = client_and_app
    user_query = "Query for no results."
    task_id = "test-internal-rag-no-results-002"
...
@pytest.mark.asyncio
@patch("apps.api.python.fastapi_v1.agents.business.internal_rag.main.MCPClient.query_agent_aggregate", new_callable=AsyncMock)
async def test_internal_rag_task_send_mcp_connection_error(client_and_app: tuple[AsyncClient, FastAPI], mocker: AsyncMock):
    client, _ = client_and_app
...
@pytest.mark.asyncio
@patch("apps.api.python.fastapi_v1.agents.business.internal_rag.main.MCPClient.query_agent_aggregate", new_callable=AsyncMock)
async def test_internal_rag_task_send_mcp_timeout_error(client_and_app: tuple[AsyncClient, FastAPI], mocker: AsyncMock):
    client, _ = client_and_app
...
@pytest.mark.asyncio
@patch("apps.api.python.fastapi_v1.agents.business.internal_rag.main.MCPClient.query_agent_aggregate", new_callable=AsyncMock)
async def test_internal_rag_task_send_mcp_generic_error(client_and_app: tuple[AsyncClient, FastAPI], mocker: AsyncMock):
    client, _ = client_and_app
...
@pytest.mark.asyncio
async def test_internal_rag_process_message_unexpected_error(client_and_app: tuple[AsyncClient, FastAPI], mocker: AsyncMock):
    client, _ = client_and_app
...
@pytest.mark.e2e
@pytest.mark.asyncio
async def test_create_and_get_internal_rag_task_e2e(client_and_app: tuple[AsyncClient, FastAPI]):
