...
from apps.api.fastapi.agents.business.metrics.main import (
    MetricsService,
...
API_V1_ROOT = Path(__file__).resolve().parents[5] # Corrected path to apps/api/
...
@pytest.mark.asyncio
@patch("apps.api.python.fastapi_v1.agents.business.metrics.main.MCPClient.query_agent_aggregate", new_callable=AsyncMock)
async def test_metrics_process_message_success(client_and_app: tuple[httpx.AsyncClient, FastAPI], mocker: AsyncMock):
    ...
    mock_query_aggregate = mocker.patch(
        "apps.api.python.fastapi_v1.agents.business.metrics.main.MCPClient.query_agent_aggregate",
