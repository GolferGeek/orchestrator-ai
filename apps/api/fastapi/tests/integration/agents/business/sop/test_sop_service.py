...
from apps.api.fastapi.agents.business.sop.main import (
    SopService, 
    AGENT_ID as SOP_AGENT_ID, 
    AGENT_NAME as SOP_AGENT_NAME, 
    AGENT_VERSION as SOP_AGENT_VERSION,
    AGENT_DESCRIPTION as SOP_AGENT_DESCRIPTION,
    MCP_TARGET_AGENT_ID as SOP_MCP_TARGET_ID,
    SOP_PRIMARY_CAPABILITY_NAME as SOP_PRIMARY_CAPABILITY,
    CONTEXT_FILE_NAME, # Added
)
from apps.api.fastapi.a2a_protocol.types import Message, TextPart, TaskSendParams, TaskState # Moved TaskState import
from apps.api.fastapi.shared.mcp.mcp_client import MCPError, MCPConnectionError, MCPTimeoutError # Added for mocking side effects
...
def load_test_context(context_file: str) -> str:
    ...
    # .parents[5] -> .../orchestrator-ai/apps/api/
...
@patch("apps.api.python.fastapi_v1.shared.mcp.mcp_client.MCPClient.query_agent_aggregate", new_callable=AsyncMock)
...
@pytest.mark.asyncio
@patch("apps.api.python.fastapi_v1.shared.mcp.mcp_client.MCPClient.query_agent_aggregate", new_callable=AsyncMock)
async def test_sop_service_task_send_valid_query(mock_query_aggregate: AsyncMock, client_and_app: tuple[httpx.AsyncClient, FastAPI]):
    client, _ = client_and_app
    user_query = "What are the steps for employee onboarding?"
    expected_response_text = "Step 1: HR sends the welcome packet. Step 2: Manager schedules introductory meetings."
    mock_query_aggregate.return_value = expected_response_text
...
@pytest.mark.asyncio
@patch("apps.api.python.fastapi_v1.shared.mcp.mcp_client.MCPClient.query_agent_aggregate", new_callable=AsyncMock)
async def test_sop_service_task_send_no_results_query(mock_query_aggregate: AsyncMock, client_and_app: tuple[httpx.AsyncClient, FastAPI]):
    client, _ = client_and_app
    user_query = "Query that returns no results"
    mock_query_aggregate.return_value = None
    
    task_params = create_simple_task_send_params(user_query, task_id="test-sop-task-no-results")
    response = await client.post("/agents/business/procedurepro/tasks", json=task_params.model_dump(mode='json'))
    ...
    assert response_data_full["status"]["state"] == TaskState.FAILED.value
    actual_response_text = response_data_full["response_message"]["parts"][0]["text"]
    expected_error_message = "No results found for the given query"
    assert actual_response_text == expected_error_message
    
    mock_query_aggregate.assert_called_once_with(
        agent_id=SOP_MCP_TARGET_ID,
        user_query=user_query,
        session_id="test-sop-task-no-results"
    )

@pytest.mark.asyncio
@patch("apps.api.python.fastapi_v1.shared.mcp.mcp_client.MCPClient.query_agent_aggregate", new_callable=AsyncMock)
async def test_sop_service_task_send_mcp_connection_error(mock_query_aggregate: AsyncMock, client_and_app: tuple[httpx.AsyncClient, FastAPI]):
    client, _ = client_and_app
    user_query = "What are the steps for employee onboarding?"
    mock_query_aggregate.side_effect = MCPConnectionError("Connection error to MCP")
...
@pytest.mark.asyncio
@patch("apps.api.python.fastapi_v1.shared.mcp.mcp_client.MCPClient.query_agent_aggregate", new_callable=AsyncMock)
async def test_sop_service_task_send_mcp_timeout_error(mock_query_aggregate: AsyncMock, client_and_app: tuple[httpx.AsyncClient, FastAPI]):
    client, _ = client_and_app
    user_query = "Query that times out"
    error_detail = "Test MCP Timeout Error Detail"
    mock_query_aggregate.side_effect = MCPTimeoutError(error_detail)
...
@pytest.mark.asyncio
@patch("apps.api.python.fastapi_v1.shared.mcp.mcp_client.MCPClient.query_agent_aggregate", new_callable=AsyncMock)
async def test_sop_service_task_send_mcp_generic_error(mock_query_aggregate: AsyncMock, client_and_app: tuple[httpx.AsyncClient, FastAPI]):
    client, _ = client_and_app
    user_query = "Query that causes generic MCP error"
    error_detail = "Test Generic MCP Error Detail"
    mock_query_aggregate.side_effect = MCPError(error_detail, status_code=500)
...
@pytest.mark.asyncio
@patch("apps.api.python.fastapi_v1.shared.mcp.mcp_client.MCPClient.query_agent_aggregate", new_callable=AsyncMock)
async def test_sop_service_task_send_unexpected_error(mock_query_aggregate: AsyncMock, client_and_app: tuple[httpx.AsyncClient, FastAPI]):
    client, _ = client_and_app
    user_query = "Query that causes unexpected error"
    error_detail = "Test Unexpected Error Detail"
    mock_query_aggregate.side_effect = Exception(error_detail)

    task_params = create_simple_task_send_params(user_query, task_id="test-sop-task-unexpected-error")
    response = await client.post("/agents/business/procedurepro/tasks", json=task_params.model_dump(mode='json'))
    ...
    mock_query_aggregate.assert_called_once_with(
        agent_id=SOP_MCP_TARGET_ID,
        user_query=user_query,
        session_id="test-sop-task-unexpected-error"
    ) 