...
class MCPClient:
    ...
    async def query_agent_aggregate(
        ...
        logger.info(f"MCPClient query_agent_aggregate starting for agent {agent_id} with query: '{user_query}'")

        try:
            async for chunk_content in self.query_agent_stream(agent_id, user_query, session_id, llm_settings, conversation_history):
                logger.info(f"MCPClient received chunk: {chunk_content}")
                chunks.append(chunk_content)
                if chunk_content.startswith("MCP Error:"):
                    logger.error(f"MCPClient received error chunk: {chunk_content}")
                    final_error_message = chunk_content
                    break # Stop processing on MCP error event
        except Exception as e:
            logger.error(f"MCPClient exception during query_agent_stream: {str(e)}", exc_info=True)
            return f"MCP Error: Exception during streaming: {str(e)}"
        
        logger.info(f"MCPClient finished streaming. Received {len(chunks)} chunks.")
        ...
        if final_error_message:
            ...
            logger.error(f"MCPClient returning error message: {final_error_message}")
            return final_error_message
        
        if not chunks and not final_error_message:
            logger.warning(f"MCPClient received no content chunks for agent {agent_id} with query: '{user_query}'")
