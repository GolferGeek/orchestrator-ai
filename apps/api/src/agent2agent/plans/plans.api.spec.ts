 ...
 * Tests mode × action architecture for plans through HTTP endpoints with auth
 *
 * These tests use the actual API endpoint: POST /agent-to-agent/:orgSlug/:agentSlug/tasks
 * and require a valid JWT token for authentication.
 */
...
describe('Plans API Integration (with Auth)', () => {
  ...
  it('should CREATE a new plan via API (action: create)', async () => {
    ...
    const response = await request(app.getHttpServer())
      ...
      .send({
        ...
        userMessage: 'Create a plan for blog post about AI',
        payload: {
    ...
    expect(response.body.payload?.content?.data?.plan.title).toBe('AI Blog Post Plan');

    createdPlanId = response.body.payload?.content?.data?.plan.id;
    versionId = response.body.payload?.content?.data?.version.id;
  });
  ...
  it('should READ the current plan via API (action: read)', async () => {
    ...
    expect(response.body.success).toBe(true);
    expect(response.body.payload?.content?.data?.id).toBe(createdPlanId);
