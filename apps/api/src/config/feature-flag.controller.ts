...
import { FeatureFlagService, FeatureFlagContext, FeatureFlagConfig } from './feature-flag.service';
...
export class FeatureFlagController {
  ...
  checkFlag(
    ...
    const enabled = this.featureFlagService.isEnabled(request.flagName, request.context || {});
  ...
  debugFlag(
    ...
    const debugInfo = {
      environmentVariables: {
        ...
        rolloutPercentage: process.env[`FEATURE_FLAG_${flagName.toUpperCase()}_ROLLOUT_PERCENTAGE`],
        targetUsers: process.env[`FEATURE_FLAG_${flagName.toUpperCase()}_TARGET_USERS`],
        targetOrganizations: process.env[`FEATURE_FLAG_${flagName.toUpperCase()}_TARGET_ORGANIZATIONS`],
        excludeUsers: process.env[`FEATURE_FLAG_${flagName.toUpperCase()}_EXCLUDE_USERS`],
        excludeOrganizations: process.env[`FEATURE_FLAG_${flagName.toUpperCase()}_EXCLUDE_ORGANIZATIONS`],
      },
    };
