...
describe('FeatureFlagService', () => {
  ...
  describe('isEnabled', () => {
    it('should return false when feature flag is disabled', () => {
      ...
      expect(configService.get).toHaveBeenCalledWith('FEATURE_FLAG_TEST_FLAG_ENABLED', 'false');
    });
    ...
    it('should respect target users', () => {
      jest.spyOn(configService, 'get').mockImplementation((key: string, defaultValue?: string) => {
        if (key === 'FEATURE_FLAG_TEST_FLAG_ENABLED') return 'true';
        if (key === 'FEATURE_FLAG_TEST_FLAG_TARGET_USERS') return 'user1,user2';
        return defaultValue || '';
      });
      
      const resultTargeted = service.isEnabled('TEST_FLAG', { userId: 'user1' });
      const resultNotTargeted = service.isEnabled('TEST_FLAG', { userId: 'user3' });
    ...
    it('should respect exclude users', () => {
      jest.spyOn(configService, 'get').mockImplementation((key: string, defaultValue?: string) => {
        if (key === 'FEATURE_FLAG_TEST_FLAG_ENABLED') return 'true';
        if (key === 'FEATURE_FLAG_TEST_FLAG_EXCLUDE_USERS') return 'user1,user2';
        return defaultValue || '';
      });
      
      const resultExcluded = service.isEnabled('TEST_FLAG', { userId: 'user1' });
      const resultNotExcluded = service.isEnabled('TEST_FLAG', { userId: 'user3' });
  ...
  describe('isSovereignRoutingEnabled', () => {
    it('should check SOVEREIGN_ROUTING flag', () => {
      ...
      expect(configService.get).toHaveBeenCalledWith('FEATURE_FLAG_SOVEREIGN_ROUTING_ENABLED', 'false');
    });
  });
});
