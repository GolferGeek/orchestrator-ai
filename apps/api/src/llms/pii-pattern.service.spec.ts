...
describe('PIIPatternService', () => {
  ...
  describe('detectPII', () => {
    ...
    it('should detect IPv4 addresses', async () => {
      ...
      const ipMatches = result.matches.filter(m => m.dataType === 'ip_address');
    ...
    it('should detect credit card numbers', async () => {
      ...
      const ccMatches = result.matches.filter(m => m.dataType === 'credit_card');
    ...
    it('should detect social media usernames', async () => {
      ...
      const usernameMatches = result.matches.filter(m => m.dataType === 'username');
    ...
    it('should detect street addresses', async () => {
      const text = 'Office is at 123 Main Street and warehouse at 456 Oak Avenue';
      
      const result = await service.detectPII(text);
      
      const addressMatches = result.matches.filter(m => m.dataType === 'address');
  ...
  describe('addCustomPattern', () => {
    it('should add custom pattern successfully', async () => {
      ...
      await expect(service.addCustomPattern(customPattern)).resolves.not.toThrow();
    });
  ...
  describe('getPatternsByDataType', () => {
    ...
    it('should return empty array for non-existent data type', () => {
      const patterns = service.getPatternsByDataType('nonexistent' as PIIDataType);
  ...
  describe('testPattern', () => {
    it('should test pattern against sample text', () => {
      const emailPattern = service.getAllPatterns().find(p => p.name === 'email_standard');
    ...
    it('should distinguish between matches and valid matches', () => {
      const ssnPattern = service.getAllPatterns().find(p => p.name === 'ssn_standard');
      ...
      if (emailMatches.length > 1) {
        ...
        for (let i = 0; i < emailMatches.length - 1; i++) {
          for (let j = i + 1; j < emailMatches.length; j++) {
            ...
            const overlap = !(match1.endIndex <= match2.startIndex || match2.endIndex <= match1.startIndex);
