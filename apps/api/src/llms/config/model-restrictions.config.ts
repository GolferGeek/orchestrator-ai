 ...
 */
export function getModelRestrictions(provider: string, modelName: string): ModelRestriction | null {
  const providerConfig = MODEL_RESTRICTIONS.find(config => config.provider === provider);
  if (!providerConfig) {
    return null;
  }
  ...
  const modelKey = Object.keys(providerConfig.models).find(key => modelName.startsWith(key));
  if (modelKey) {
...
export function modelSupports(
  ...
  if (featureConfig && typeof featureConfig === 'object' && 'supported' in featureConfig) {
...
export function applyModelRestrictions(
  ...
  } else if (restrictions.temperature && adjustedConfig.temperature !== undefined) {
    // Clamp temperature to valid range
    const min = restrictions.temperature.min ?? 0;
    const max = restrictions.temperature.max ?? 2;
    adjustedConfig.temperature = Math.max(min, Math.min(max, adjustedConfig.temperature));
  }
