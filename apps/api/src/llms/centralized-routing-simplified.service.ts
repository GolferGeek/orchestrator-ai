...
export class SimplifiedCentralizedRoutingService {
  ...
  async route(
    ...
    try {
      ...
      if (hasShowstoppers) {
        this.logger.warn('🛑 [ROUTING] Showstopper PII detected - blocking request');
      ...
      if (sovereignMode) {
        ...
        this.logger.debug('🔒 [ROUTING] Sovereign mode active - using local model');
      }
      ...
      this.logger.debug(`✅ [ROUTING] Decision: ${provider}/${model} (local: ${isLocal})`);
      this.logger.debug(`✅ [ROUTING] PII: ${metadata.flagCount} flags detected`);
      ...
      this.logger.error(`❌ [ROUTING] Error: ${error instanceof Error ? error.message : String(error)}`);
  ...
  async processResponse(
    ...
    this.logger.debug(`🔄 [ROUTING] Reversed ${result.reversalCount} pseudonyms in response`);
