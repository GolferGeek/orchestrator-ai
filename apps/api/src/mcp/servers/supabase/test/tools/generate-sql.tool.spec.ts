...
describe('GenerateSQLTool', () => {
  ...
  beforeAll(async () => {
    ...
    contextService = moduleRef.get<ContextLearningService>(
      ContextLearningService,
    );
    ...
    test.each(easyTests.map((tc) => [tc.id, tc]))(
      ...
      async (id: string, testCase: TestPromptCase) => {
        ...
        testCase.expectedFeatures.forEach((feature) => {
          expect(result.content.sql.toUpperCase()).toContain(
            feature.toUpperCase(),
          );
        });

        // Validate SQL syntax
        const validation = await testEnv.sqlValidator.validateSQL(
          result.content.sql,
        );
  ...
  describe('Advanced SQL Generation', () => {
    const advancedTests = TEST_PROMPTS.filter(
      (tc) => tc.complexity === 'advanced',
    );
    ...
    test('should handle recursive CTEs when appropriate', async () => {
      const prompt =
        'Find conversation threads with recursive task relationships';
      const result = await generateSQLFromPrompt(prompt);
  ...
  describe('Context Learning Integration', () => {
    ...
    test('should improve after learning from successful patterns', async () => {
      ...
      const enhanced = await contextService.enhancePrompt(
        prompt,
        'generate-sql',
      );
      ...
      expect(result.content.sql).toMatch(
    ...
    test('should avoid patterns marked as errors in context', async () => {
      ...
      expect(enhanced.warnings.some((w) => w.includes('created_at'))).toBe(
        true,
      );
    ...
    test('should apply advanced patterns for complex queries', async () => {
      ...
      expect(
        enhanced.appliedPatterns.some((p) => p.type === 'optimization'),
      ).toBe(true);
  ...
  async function generateSQLWithMockLLM(
    prompt: string,
    options: any,
    mockLLM: jest.Mock,
  ): Promise<any> {
  ...
  function generateMockSQL(prompt: string): string {
    ...
    if (
      ...
      lowerPrompt.includes('get users')
    ) {
    ...
    if (
      lowerPrompt.includes('window functions') ||
