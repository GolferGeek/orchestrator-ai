...
export class SQLValidator {
    ...
    try {
      ...
      if (!validation.isReadOnly && !options.dryRun) {
        return {
          success: false,
          error:
            'Write operations require dry_run=false and explicit confirmation',
        };
      }
  ...
  private estimateComplexity(sql: string): 'low' | 'medium' | 'high' {
    ...
    if (
      upperSQL.includes('SUBQUERY') ||
      (upperSQL.includes('SELECT') &&
        upperSQL.indexOf('SELECT') !== upperSQL.lastIndexOf('SELECT'))
    )
      complexity += 2;
    if (upperSQL.includes('UNION')) complexity += 2;
    if (upperSQL.includes('WITH') || upperSQL.includes('CTE')) complexity += 2;
    if (upperSQL.includes('WINDOW') || upperSQL.includes('OVER'))
      complexity += 3;
  ...
  private validateSecurity(sql: string, result: SQLValidationResult): void {
    ...
    if (
      /\b(version|current_user|session_user|current_database)\(\)/i.test(sql)
    ) {
      ...
      if (!this.KNOWN_TABLES.includes(table)) {
        ...
        try {
          ...
          result.warnings.push(
            `Could not verify existence of table '${table}'`,
          );
        }
      }
    }
  ...
  private validateColumnReferences(
    ...
    if (/created_date|updated_date/i.test(sql)) {
      result.warnings.push(
        'Use "created_at" and "updated_at" instead of "created_date" and "updated_date"',
      );
    }
    ...
    if (/JOIN/i.test(sql) && !/\w+\.\w+/.test(sql)) {
      result.warnings.push(
        'Consider using table prefixes in JOIN queries for clarity',
      );
    }
  }
  ...
  async testSQLQuery(
    ...
    if (expectedColumns && executionResult.success && executionResult.data) {
      ...
      const firstRow = Array.isArray(executionResult.data)
        ? executionResult.data[0]
        : executionResult.data;
      if (firstRow && typeof firstRow === 'object') {
        const actualColumns = Object.keys(firstRow);
        hasExpectedStructure = expectedColumns.every((col) =>
