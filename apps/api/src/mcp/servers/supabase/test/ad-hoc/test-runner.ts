...
class AdHocTestRunner {
  ...
  async initialize() {
    ...
    // Check migrations
    const { hasMCPTables, missingTables } =
      await this.testSetup.checkMigrations();
    if (!hasMCPTables) {
  ...
  async runTestCase(testCase: TestPromptCase): Promise<TestResult> {
    ...
    if (result.success) {
      ...
      if (result.validationResult?.errors) {
        console.log(
          `🔍 Validation errors: ${result.validationResult.errors.join(', ')}`,
        );
      }
    }
  ...
  async createTestData(): Promise<void> {
    ...
    try {
      ...
      const conversation =
        await env.testDataManager.createTestAgentConversation(user.id, {
      ...
      const execution = await env.testDataManager.createTestMCPExecution(
        user.id,
        {
