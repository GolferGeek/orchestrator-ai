...
export class SupabaseMCPServer
  extends IntelligentMCPBaseService
  implements IMCPServer, OnModuleInit, OnModuleDestroy
{
  ...
  protected serverInfo: MCPServerInfo = {
    ...
    version: '2.0.0',
    description:
      'Intelligent Supabase MCP server with context learning and execution tracking',
    capabilities: {
      tools: true,
      resources: true,
      prompts: true,
  ...
  constructor(
    ...
    const workingTracker =
      executionTracker || new MCPExecutionTrackerService(supabaseService);
  ...
  async initialize(config: SupabaseMCPConfig): Promise<void> {
    try {
      ...
      if (
        ...
        typeof this.executionTracker.setSupabaseClient === 'function'
      ) {
  ...
  private async testConnection(): Promise<void> {
    try {
      ...
      throw new Error(
        `Failed to verify database connection: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
    }
  }
  ...
  protected async executeToolImplementation(
    ...
    console.log(
      `🔧 TOOL EXECUTION DEBUG: this.supabaseClient = ${!!this.supabaseClient}`,
    );
    console.log(
      `🔧 TOOL EXECUTION DEBUG: Parameters:`,
    ...
    const enhancedOptions = {
      ...options,
      llmProvider:
        options.llmProvider || this.config.defaultLLMProvider || 'anthropic',
      llmModel:
        options.llmModel || this.config.defaultLLMModel || 'claude-3-5-sonnet',
    };
    ...
    try {
      ...
      switch (toolName) {
        case 'generate-sql':
          ...
          const result = await this.generateSQLTool.execute(
            parameters,
            enhancedOptions,
          );
          console.log(
            '🚀 SERVER DEBUG: Result from generateSQLTool.execute completed',
          );
          return result;
        ...
        case 'get-schema':
          ...
          console.log(
            `🔧 TOOL DEBUG: getSchemaTool exists = ${!!this.getSchemaTool}`,
          );
        ...
        case 'execute-sql':
          ...
          console.log(
            `🔧 TOOL DEBUG: executeSQLTool exists = ${!!this.executeSQLTool}`,
          );
          return await this.executeSQLTool.execute(parameters, enhancedOptions);

        case 'query-and-format':
          console.log(
            '🔧 TOOL DEBUG: About to call queryAndFormatTool.execute',
          );
          return await this.queryAndFormatTool.execute(
            parameters,
            enhancedOptions,
          );
  ...
  private async sendHeartbeat(): Promise<void> {
    try {
      ...
      this.logger.debug(
        'Heartbeat mechanism disabled - using direct MCP Client Service',
      );
    } catch (error) {
      this.logger.warn(
        `Heartbeat mechanism disabled: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
    }
  }
  ...
  private async collectMetrics(): Promise<any> {
    try {
      ...
      if (
        this.executionTracker &&
        typeof this.executionTracker.getExecutionStats === 'function'
      ) {
        try {
          const trackerStats = await this.executionTracker.getExecutionStats(
            'system',
            30,
          );
          const successfulExecutions = Math.round(
            (trackerStats.successRate / 100) * trackerStats.totalExecutions,
          );
          const failedExecutions =
            trackerStats.totalExecutions - successfulExecutions;
          ...
          executionStats = {
            ...
            toolsUsed:
              trackerStats.topTools?.reduce((acc: any, tool: any) => {
  ...
  async healthCheck(): Promise<{
    ...
    try {
      ...
      const dbStatus =
        error && !error.message.includes('relation') ? 'unhealthy' : 'healthy';

      // Test context learning
      const contextStats = this.contextLearning?.getContextStats() || {
        totalPatterns: 0,
        lastReload: null,
      };
  ...
  async getContextStats(): Promise<any> {
    return (
      this.contextLearning?.getContextStats() || {
        totalPatterns: 0,
        lastReload: null,
      }
    );
  ...
  async callTool(
    ...
    try {
      console.log(
        '🎯 CALLTOOL DEBUG: callTool method called with:',
        request.name,
      );
      console.log(
        '🎯 CALLTOOL DEBUG: arguments:',
      ...
      const executionResult = await this.executeTool(
        ...
        {
          userId:
            this.configService?.get<string>('SUPABASE_TEST_USERID') ||
      ...
      prompts: [
        {
          ...
          description:
            'Generate an optimized SQL query with performance considerations',
          arguments: [
            {
            ...
            },
            {
              ...
              description: 'Performance optimization level (low, medium, high)',
              required: false,
            },
          ],
        },
        {
          ...
          description:
            'Analyze and suggest improvements for an existing SQL query',
          arguments: [
            {
              name: 'sql_query',
  ...
  async getPrompt(request: MCPGetPromptRequest): Promise<MCPGetPromptResponse> {
    if (request.name === 'generate-optimized-sql') {
      const queryDescription =
        request.arguments?.query_description || 'data retrieval query';
