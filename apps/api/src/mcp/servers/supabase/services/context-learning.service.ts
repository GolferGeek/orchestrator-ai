...
export class ContextLearningService {
  ...
  private contextCache: Map<string, { content: string; lastModified: Date }> =
    new Map();
  ...
  constructor() {
    ...
    console.log(
      `📚 Context Learning Service tools directory: ${this.toolsDirectory}`,
    );
  }
  ...
  async getContext(toolName: string): Promise<string> {
    ...
    try {
      ...
      console.log(
        `📚 Loaded context for tool '${toolName}' (${content.length} chars)`,
      );
      ...
      if ((error as NodeJS.ErrnoException).code === 'ENOENT') {
        ...
        console.warn(
          `⚠️ Failed to load context for tool '${toolName}':`,
          error,
        );
      }
      return '';
  ...
  async enhancePrompt(
    ...
    if (!context) {
      console.log(
        `📚 No context found for tool '${toolName}', using original prompt`,
      );
...
${originalPrompt}`;
    ...
    console.log(
      `📚 Enhanced prompt for '${toolName}' (${enhancedPrompt.length} chars total)`,
    );
    console.log(
      `📚 Enhanced prompt preview: ${enhancedPrompt.substring(0, 300)}...`,
    );
