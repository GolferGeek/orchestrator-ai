...
export class EnhancedGetSchemaTool {
  private schemaCache: Map<string, { data: any; timestamp: number }> =
    new Map();
  ...
  private async getFullSchema(): Promise<any> {
    ...
    try {
      ...
      console.log(
        `Schema built successfully with ${this._fullSchema.tables.length} tables`,
      );
      return this._fullSchema;
    } finally {
  ...
  private async buildSchemaFromDatabase(): Promise<any> {
    ...
    try {
      ...
      const serviceRoleKey =
        ...
        (this.supabaseClient as any).supabaseKey;

      console.log(
        `Using key ending in: ${serviceRoleKey.slice(-10)} (length: ${serviceRoleKey.length})`,
      );
      ...
      console.log(
        `Key type: ${isServiceRole ? 'SERVICE_ROLE' : isAnon ? 'ANON' : 'UNKNOWN'}`,
      );
      ...
      console.log(
        `✓ Found ${tableNames.length} tables from OpenAPI schema: ${tableNames.join(', ')}`,
      );
      console.log(
        `DEBUG: Total paths in OpenAPI: ${Object.keys(paths).length}`,
      );
      ...
      console.log(
        `Discovered ${tableNames.length} tables, now getting column information...`,
      );
      ...
      for (const tableName of tableNames) {
        try {
          ...
          if (tableDefinition && tableDefinition.properties) {
            ...
            columns = Object.keys(tableDefinition.properties).map(
              (columnName) => {
                ...
                return {
                  ...
                  nullable:
                    ...
                    !tableDefinition.required.includes(columnName),
            ...
            console.log(
              `✓ Found ${columns.length} columns for ${tableName} from OpenAPI definition`,
            );
          } else {
            console.log(
              `No OpenAPI definition for ${tableName}, trying sample data approach...`,
            );

            // Fallback: try to get column info from sample data
            try {
              const { data: sampleData, error: sampleError } =
                await this.supabaseClient.from(tableName).select('*').limit(1);
              ...
              if (!sampleError && sampleData && sampleData.length > 0) {
                ...
                console.log(
                  `✓ Inferred ${columns.length} columns for ${tableName} from sample data`,
                );
              } else {
                ...
                console.log(
                  `⚠️ Using minimal schema for ${tableName} - table exists but no accessible data`,
                );
              }
            } catch (accessError) {
              ...
              console.log(
                `⚠️ Table ${tableName} exists but access restricted - including with minimal schema`,
              );
            }
          }
          ...
          console.error(
            `Error getting schema for table ${tableName}:`,
            tableError,
          );
      ...
      console.log(
        `Successfully built complete schema with ${discoveredTables.length} tables`,
      );
      console.log(
        `Final table list: ${discoveredTables.map((t) => t.name).join(', ')}`,
      );
      ...
      throw new Error(
        `Failed to build schema from database: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
    }
  }
  ...
  private mapOpenAPIType(type: string, format?: string): string {
    ...
    if (lowerFormat === 'date-time' || lowerFormat === 'date')
      return 'timestamp';
   ...
   */
  private analyzeTablePurpose(
    tableName: string,
    columns: any[],
  ): { purpose: string; businessContext: string } {
    ...
    if (name.includes('user')) {
      return {
        purpose:
          'User management, authentication, and user-related data storage',
    ...
    if (name.includes('kpi')) {
      return {
        purpose:
          'Key Performance Indicator tracking, metrics, and business analytics',
    ...
    if (name.includes('llm') || name.includes('model')) {
      return {
        purpose:
          'Large Language Model configuration, usage tracking, and management',
    ...
    if (name.includes('company') || name.includes('department')) {
      return {
        purpose:
          'Organizational structure, company data, and department management',
    ...
    if (name.includes('provider')) {
      return {
        purpose:
          'Service provider configuration and external system management',
    ...
    if (
      columnNames.includes('metric_id') ||
      columnNames.includes('value') ||
      columnNames.includes('target_value')
    ) {
      return {
    ...
    if (
      columnNames.includes('execution_time') ||
      columnNames.includes('status')
    ) {
      return {
        purpose:
          'System execution monitoring, status tracking, and performance measurement',
    ...
    if (
      ...
      columnNames.includes('configuration')
    ) {
      return {
