...
export class EnhancedReadDataTool {
  ...
  async execute(
    ...
    try {
      const query = this.supabaseClient.from(parameters.table_name);
      ...
      if (error) {
        throw new Error(
          `Failed to read from ${parameters.table_name}: ${error.message}`,
        );
      }
      ...
      throw new Error(
        `Read operation failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
    }
  }
}
