...
export class MCPModule {
  ...
  private async initializeModule(): Promise<void> {
    try {
      ...
      console.log(`✅ MCP Server initialized: ${serverInfo.serverInfo.name} v${serverInfo.serverInfo.version}`);
      console.log(`📋 Protocol version: ${serverInfo.protocolVersion}`);
      
      // List available tools for debugging
      const toolsResult = await this.mcpService.listTools();
      const namespaces = [...new Set(toolsResult.tools.map(tool => tool.name.split('/')[0]))];
      
      console.log(`🔧 ${toolsResult.tools.length} tools loaded across ${namespaces.length} namespaces: ${namespaces.join(', ')}`);
      ...
      Object.entries(pingResult.namespaces || {}).forEach(([namespace, healthy]) => {
        const status = healthy ? '✅' : '❌';
        console.log(`   ${status} ${namespace}: ${healthy ? 'healthy' : 'unhealthy'}`);
      });
