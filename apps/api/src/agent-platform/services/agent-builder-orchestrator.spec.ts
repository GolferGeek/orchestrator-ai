...
describe('Agent Builder Orchestrator (Full Flow)', () => {
  ...
  describe('Step 1: Intent Collection', () => {
    it('should prompt for agent type and purpose when no data provided', async () => {
      ...
      expect(res.result?.content).toContain('What type of agent');
      expect(res.result?.needsInput).toBe(true);
      expect(res.result?.state?.step).toBe('intent');
    });
    ...
    it('should collect intent and move to basic_info', async () => {
      ...
      expect(res.result?.state?.agentConfig?.agent_type).toBe('function');
  ...
  describe('Step 2: Basic Info', () => {
    it('should require slug and displayName', async () => {
      ...
      expect(res.ok).toBe(true);
      expect(res.result?.content).toContain('required fields');
      expect(res.result?.needsInput).toBe(true);
    });
    ...
    it('should collect basic info and move to io_contract', async () => {
      ...
      expect(res.result?.state?.agentConfig?.slug).toBe('doc_processor');
      expect(res.result?.content).toContain('IO Contract');
    });
  });
  ...
  describe('Step 3: IO Contract', () => {
    it('should require both input and output modes', async () => {
      ...
      expect(res.ok).toBe(true);
      expect(res.result?.content).toContain('both inputModes and outputModes');
    });
    ...
    it('should collect IO contract and move to agent_config', async () => {
      ...
      expect(res.result?.state?.agentConfig?.yaml).toContain('input_modes');
      expect(res.result?.content).toContain('Function Configuration');
    });
    ...
    it('should show context prompt for context agents', async () => {
      ...
      expect(res.ok).toBe(true);
      expect(res.result?.content).toContain('Context Configuration');
      expect(res.result?.content).toContain('system prompt');
    });
  });
  ...
  describe('Step 4: Agent Configuration', () => {
    it('should require code and timeout for function agents', async () => {
      ...
      expect(res.ok).toBe(true);
      expect(res.result?.content).toContain('both code and timeoutMs');
    });
