...
describe('Orchestrator Integration - Complete Real LLM Workflow', () => {
  ...
  beforeEach(async () => {
    ...
    intentService = module.get<IntentRecognitionService>(IntentRecognitionService);
  ...
  describe('Complete Plan-Approve-Act Workflow', () => {
    ...
    it('should execute complete marketing campaign workflow with real LLM intelligence', async () => {
      ...
      const planRequest: OrchestratorInput = {
        prompt: "Create a comprehensive marketing campaign for our new AI product launch. Include market research, blog content creation, social media strategy, and competitive analysis over 6 weeks.",
        userId: "test-user-integration",
        conversationId: "test-conv-integration",
        conversationHistory: []
      };

      const intentResult: IntentDirective = await intentService.classifyIntent(planRequest);
      ...
      console.log(`✅ Intent Classification: ${intentResult.action} (confidence: ${intentResult.confidence})`);
      console.log(`🧠 LLM Reasoning: ${intentResult.reasoning.substring(0, 150)}...`);

      // PHASE 2: PROJECT PLANNING (if CREATE_PROJECT)
      if (intentResult.action === 'CREATE_PROJECT') {
        console.log('\n📊 PHASE 2: PROJECT PLANNING');
        
        const projectPlan: PlanDefinition = await planningService.createPlan(planRequest);
        ...
        console.log(`🎯 First Step: ${projectPlan.steps[0]?.stepName} (${projectPlan.steps[0]?.agentName})`);
        ...
        projectPlan.steps.forEach((step, index) => {
          ...
          console.log(`  📌 Step ${index + 1}: ${step.stepName} → ${step.agentName || 'Human'}`);
        });

        // PHASE 3: PLAN REFINEMENT
        console.log('\n🔄 PHASE 3: PLAN REFINEMENT');
        
        const refinementFeedback = "I want to add influencer outreach and email marketing campaigns to amplify our reach. Also include A/B testing for the landing pages.";
        ...
        expect(refinedPlan.steps.length).toBeGreaterThanOrEqual(projectPlan.steps.length);
        console.log(`✅ Plan Refined: ${refinedPlan.steps.length} steps (was ${projectPlan.steps.length})`);
        ...
        console.log(`🧠 LLM Successfully Incorporated: influencer outreach, email marketing, A/B testing`);
      }
      ...
      if (intentResult.action === 'DELEGATE' && intentResult.agentName) {
        ...
        console.log(`📊 Delegation Result: ${delegationResult.response?.substring(0, 150) || 'No response'}...`);
      }
    ...
    it('should handle multi-step conversation with context awareness', async () => {
      ...
      const step2Request: OrchestratorInput = {
        prompt: "Actually, let's focus specifically on content marketing and thought leadership to establish our expertise in the AI space",
        userId: "test-user-context",
        conversationId: "test-conv-context",
        conversationHistory: [
          {
          ...
          {
            role: 'assistant',
            content: 'I can help you develop a comprehensive marketing strategy.',
            timestamp: new Date().toISOString(),
      ...
      console.log(`🧠 Context Awareness: ${step2Intent.reasoning.substring(0, 150)}...`);

      // STEP 3: Specific task request
      const step3Request: OrchestratorInput = {
        prompt: "Create a blog post about how AI is transforming project management workflows",
        userId: "test-user-context", 
        conversationId: "test-conv-context",
        conversationHistory: [
    ...
    it('should create sophisticated project plans with proper dependencies', async () => {
      ...
      const hasResearchStep = complexPlan.steps.some(s => 
        s.stepName.toLowerCase().includes('research') || s.stepName.toLowerCase().includes('analysis')
      );
      const hasApprovalSteps = complexPlan.steps.some(s => s.stepType === 'human_approval');
      const hasBrandingSteps = complexPlan.steps.some(s => 
        s.stepName.toLowerCase().includes('brand') || s.stepName.toLowerCase().includes('identity')
      );
      ...
      complexPlan.steps.forEach(step => {
        step.dependencies.forEach(depId => {
          ...
          if (!depExists) {
            ...
            console.log(`❌ Invalid dependency: ${step.stepId} depends on ${depId}`);
          }
        });
      });
      ...
      console.log(`🧠 LLM Created Sophisticated Multi-Step Plan with Proper Dependencies`);
