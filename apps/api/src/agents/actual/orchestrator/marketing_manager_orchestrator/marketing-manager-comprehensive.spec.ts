...
describe('Marketing Manager Orchestrator - Comprehensive LLM Tests', () => {
  ...
  beforeEach(async () => {
    ...
    (marketingManager as any).agentPath = 'orchestrator/marketing_manager_orchestrator';
  ...
  describe('Single Agent Delegation Intelligence', () => {
    ...
    it('should intelligently delegate blog post creation to blog_post agent', async () => {
      ...
      const blogRequest = {
        prompt: "Write a comprehensive blog post about 'The Future of AI in Customer Service' targeting business decision makers. Include practical examples and actionable insights.",
        userId: "test-marketing-blog",
        conversationId: "test-conv-blog",
        conversationHistory: []
      };

      const result = await marketingManager.executeTask('executeTask', blogRequest);
      ...
      expect(['blog_post', 'content', 'marketing_swarm']).toContain(result.agentName);
    ...
    it('should intelligently delegate competitive analysis to competitors agent', async () => {
      ...
      const competitorRequest = {
        prompt: "Analyze our main competitors in the AI automation space. Focus on their pricing strategies, feature positioning, and market messaging. Provide a SWOT analysis comparing us to the top 3 competitors.",
        userId: "test-marketing-competitors",
        conversationId: "test-conv-competitors", 
        conversationHistory: []
      };

      const result = await marketingManager.executeTask('executeTask', competitorRequest);
    ...
    it('should intelligently delegate market research to market_research agent', async () => {
      ...
      const researchRequest = {
        prompt: "Conduct market research on small business adoption of AI tools. What are the main barriers to adoption? What features do they value most? Include demographic insights and buying behavior patterns.",
        userId: "test-marketing-research",
        conversationId: "test-conv-research",
        conversationHistory: []
      };

      const result = await marketingManager.executeTask('executeTask', researchRequest);
    ...
    it('should intelligently delegate general content creation to content agent', async () => {
      ...
      const contentRequest = {
        prompt: "Create compelling marketing copy for our new AI product landing page. Include a powerful headline, benefit-focused subheadings, and a strong call-to-action. Target audience: tech-savvy small business owners.",
        userId: "test-marketing-content",
        conversationId: "test-conv-content",
        conversationHistory: []
      };

      const result = await marketingManager.executeTask('executeTask', contentRequest);
  ...
  describe('Multi-Step Marketing Workflow Intelligence', () => {
    ...
    it('should handle simple direct delegation requests without clarification', async () => {
      ...
      const simpleRequests = [
        {
          prompt: "Write a blog post about AI project management benefits for small businesses",
    ...
    it('should handle complex requests with clarification and proper A/B choices', async () => {
      ...
      const complexRequests = [
        {
          prompt: "Develop a comprehensive competitive positioning strategy that differentiates us from major players in the AI space while creating targeted messaging for multiple customer segments",
          testChoice: 'A', // Test delegation choice
          description: "Complex competitive strategy"
        },
        {
          prompt: "Create an integrated content marketing approach that spans blog posts, social media, email campaigns, and website updates for our product launch",
          testChoice: 'B', // Test project choice  
          description: "Multi-channel content strategy"
        },
        {
          prompt: "Launch a comprehensive market research initiative to understand customer needs, competitive landscape, and pricing optimization across different market segments",
          testChoice: 'A', // Test delegation choice
      ...
      for (let i = 0; i < complexRequests.length; i++) {
        ...
        const clarificationResult = await marketingManager.executeTask('executeTask', {
          prompt: request.prompt,
        ...
        if (clarificationResult.action === 'CLARIFY') {
          ...
          const choiceResult = await marketingManager.executeTask('executeTask', {
          ...
          if (request.testChoice === 'A') {
            ...
            console.log(`✅ Choice A → Delegated to: ${choiceResult.agentName}`);
          } else {
          ...
          console.log(`ℹ️ Direct delegation instead of clarification: ${clarificationResult.agentName}`);
    ...
    it('should orchestrate complete product launch campaign across all marketing agents', async () => {
      ...
      const researchRequest = {
        prompt: "Research the market landscape for AI-powered project management tools. Who are our competitors? What's the market size? What are customer pain points?",
        userId: "test-product-launch",
        conversationId: "test-conv-launch-research",
        conversationHistory: []
      };

      const researchResult = await marketingManager.executeTask('executeTask', researchRequest);
      ...
      const competitorRequest = {
        prompt: "Analyze our top 3 competitors in the AI project management space. I need a competitive analysis report with their strengths, weaknesses, and how we differentiate.",
        userId: "test-product-launch",
      ...
      const competitorResult = await marketingManager.executeTask('executeTask', competitorRequest);
      ...
      if (competitorResult.action === 'CLARIFY' && competitorResult.requiresUserChoice) {
        console.log('🔄 Phase 2 triggered clarification, choosing delegation...');
        const clarificationChoice = {
          ...
          conversationHistory: [
            ...
            },
            {
              ...
              timestamp: new Date().toISOString(),
              metadata: { agentName: 'Orchestrator', action: 'CLARIFY', requiresUserChoice: true }
            }
          ]
        };
        finalCompetitorResult = await marketingManager.executeTask('executeTask', clarificationChoice);
      }
      
      console.log(`✅ Positioning completed by: ${finalCompetitorResult.agentName || 'orchestrator'}`);

      // PHASE 3: Content Strategy & Creation
      console.log('\n✍️ PHASE 3: Content Creation');
      const contentRequest = {
        prompt: "Write marketing copy for our AI project management tool launch. I need website copy, email content, and social media posts.",
        userId: "test-product-launch",
      ...
      const contentResult = await marketingManager.executeTask('executeTask', contentRequest);
      expect(contentResult.success).toBe(true);
      
      // Handle potential clarification response
      let finalContentResult = contentResult;
      if (contentResult.action === 'CLARIFY' && contentResult.requiresUserChoice) {
        console.log('🔄 Phase 3 triggered clarification, choosing delegation...');
        const clarificationChoice = {
          prompt: "A", // Choose delegation
          userId: "test-product-launch",
          conversationId: "test-conv-launch-content",
        ...
        finalContentResult = await marketingManager.executeTask('executeTask', clarificationChoice);
      }
      
      console.log(`✅ Content created by: ${finalContentResult.agentName || 'orchestrator'}`);

      // PHASE 4: Blog Content for Thought Leadership
      console.log('\n📝 PHASE 4: Blog Content Creation');
      const blogRequest = {
        prompt: "Write a thought leadership blog post that positions us as experts in AI project management. Use insights from our research and positioning work.",
        userId: "test-product-launch",
      ...
      const blogResult = await marketingManager.executeTask('executeTask', blogRequest);
      ...
      const launchRequest = {
        prompt: "Write a final marketing summary for our AI project management tool launch. I need a brief overview of our key messages and next steps.",
        userId: "test-product-launch",
      ...
      const launchResult = await marketingManager.executeTask('executeTask', launchRequest);
      expect(launchResult.success).toBe(true);
      
      // Handle potential clarification response
      let finalLaunchResult = launchResult;
      if (launchResult.action === 'CLARIFY' && launchResult.requiresUserChoice) {
        console.log('🔄 Phase 5 triggered clarification, choosing delegation...');
        const clarificationChoice = {
          prompt: "A", // Choose delegation
          userId: "test-product-launch",
          conversationId: "test-conv-launch-campaign",
        ...
        finalLaunchResult = await marketingManager.executeTask('executeTask', clarificationChoice);
      }
      
      console.log(`✅ Campaign coordinated by: ${finalLaunchResult.agentName || 'orchestrator'}`);
      ...
      console.log('🧠 Marketing Manager demonstrated intelligent agent coordination across full campaign lifecycle');
  ...
  describe('Context-Aware Agent Selection Intelligence', () => {
    ...
    it('should demonstrate intelligent agent selection based on conversation context', async () => {
      ...
      const step1 = {
        prompt: "I need to understand our target market better. What demographics should we focus on for our AI product?",
      ...
      const step2 = {
        prompt: "Now I want to know how our competitors are targeting those same demographics. What's their messaging strategy?",
        userId: "test-context-aware",
        conversationId: "test-conv-context",
        conversationHistory: [
          {
      ...
      const step3 = {
        prompt: "Based on what we learned, create compelling ad copy that highlights our advantages over competitors",
        userId: "test-context-aware",
        conversationId: "test-conv-context",
        conversationHistory: [
      ...
      const step4 = {
        prompt: "Now let's launch this as a coordinated multi-channel campaign across social media, email, and paid ads",
        userId: "test-context-aware",
        conversationId: "test-conv-context",
        conversationHistory: [
      ...
      const agentSequence = [result1.agentName, result2.agentName, result3.agentName, result4.agentName];
      ...
      expect(agentSequence[3]).toMatch(/(content|blog_post|market_research|competitors)/); // Campaign coordination
      
      console.log('🎯 Marketing Manager demonstrated intelligent context-aware agent selection!');
  ...
  describe('Marketing Manager Intelligence Validation', () => {
    ...
    it('should handle complete clarification workflow from complex request to final execution', async () => {
      ...
      const complexRequest = {
        prompt: "Launch a comprehensive multi-channel marketing campaign for our Q2 product release. Coordinate social media, email marketing, content marketing, and paid advertising.",
        userId: "test-clarify-flow",
        conversationId: "test-conv-clarify-flow",
        conversationHistory: []
      };

      const clarificationResult = await marketingManager.executeTask('executeTask', complexRequest);
      ...
      console.log(`✅ Clarification received: ${clarificationResult.response?.substring(0, 100)}...`);
      console.log(`🤖 Option A (Delegate): ${clarificationResult.options.delegate.agentName}`);
      console.log(`📋 Option B (Project): ${clarificationResult.options.project.outline}`);
      ...
      const delegationResult = await marketingManager.executeTask('executeTask', choiceRequest);
      ...
      console.log(`📊 Final response: ${delegationResult.response?.substring(0, 200)}...`);
      
      // Validate that we got actual agent delegation, not just conversation
      expect(['content', 'blog_post', 'competitors', 'market_research']).toContain(delegationResult.agentName);
      
      console.log('\n🎉 Complete Clarification Workflow Successful!');
      console.log('✅ Complex request → Clarification → User choice → Agent delegation');
    ...
    it('should handle clarification workflow with project creation choice', async () => {
      ...
      const complexRequest = {
        prompt: "Create a comprehensive product launch strategy with multiple marketing touchpoints and coordination across teams.",
      ...
      const projectResult = await marketingManager.executeTask('executeTask', projectChoiceRequest);
      ...
      console.log(`📊 Result: ${projectResult.success ? 'Success' : 'Expected failure due to test environment'}`);
