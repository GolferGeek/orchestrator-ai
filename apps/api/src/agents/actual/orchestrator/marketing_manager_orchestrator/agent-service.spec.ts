...
describe('MarketingManagerOrchestratorService - Complete LLM Workflow Tests', () => {
  ...
  beforeEach(async () => {
    ...
    service = module.get<MarketingManagerOrchestratorService>(MarketingManagerOrchestratorService);
  });
  ...
  describe('LLM Intent Recognition for Marketing Requests', () => {
    ...
    it('should correctly classify simple content request as delegation', async () => {
      const input: OrchestratorInput = {
        prompt: "I need a blog post about the future of AI in project management",
        userId: "test-user",
      ...
      const response: OrchestratorResponse = await service.executeTask('executeTask', input);

      expect(response.success).toBe(true);
      expect(response.metadata?.action).toMatch(/(DELEGATE|delegate)/i);
      // Should have routed to a specific agent
      expect(response.agentName || response.metadata?.delegatedAgent).toBeDefined();
    });
    ...
    it('should classify complex marketing campaign as project creation', async () => {
      const input: OrchestratorInput = {
        prompt: "I want to launch a comprehensive marketing campaign for our new AI product. This needs market research, competitor analysis, blog content, social media posts, email sequences, and coordinated timing over 2 months.",
        userId: "test-user",
        conversationId: "test-conv",
        conversationHistory: []
      };
    ...
    it('should handle strategic marketing questions as conversation', async () => {
      const input: OrchestratorInput = {
        prompt: "What are the current trends in B2B marketing? I'm trying to understand what strategies work best for SaaS companies.",
        userId: "test-user",
        conversationId: "test-conv",
        conversationHistory: []
      };
  ...
  describe('LLM Agent Selection Intelligence', () => {
    ...
    it('should delegate blog content to blog_post writer', async () => {
      const input: OrchestratorInput = {
        prompt: "Create a comprehensive 2000-word thought leadership article about AI transforming project management, with SEO optimization and industry insights",
        userId: "test-user",
        conversationId: "test-conv",
        conversationHistory: []
      };
      ...
      const delegatedAgent = response.agentName || response.metadata?.delegatedAgent;
    ...
    it('should delegate promotional copy to content writer', async () => {
      const input: OrchestratorInput = {
        prompt: "Write compelling copy for our product landing page including headlines, feature descriptions, benefits, and call-to-action buttons",
        userId: "test-user", 
        conversationId: "test-conv",
        conversationHistory: []
      };
    ...
    it('should delegate market analysis to research agents', async () => {
      const input: OrchestratorInput = {
        prompt: "I need a detailed competitive analysis of the top 5 project management tools, including pricing, features, and market positioning",
        userId: "test-user",
        conversationId: "test-conv", 
        conversationHistory: []
      };
    ...
    it('should delegate complex campaigns to marketing swarm', async () => {
      const input: OrchestratorInput = {
        prompt: "Launch a complete product announcement campaign with press release, multiple blog posts, social media content across platforms, email sequences, and landing page copy",
        userId: "test-user",
        conversationId: "test-conv",
        conversationHistory: []
      };
  ...
  describe('LLM Project Planning Intelligence', () => {
    ...
    it('should create comprehensive marketing campaign project plans', async () => {
      const input: OrchestratorInput = {
        prompt: "Plan a 6-week product launch campaign for our new AI-powered analytics dashboard. Target enterprise customers, include thought leadership content, competitive positioning, lead generation, and coordinated launch activities.",
        userId: "test-user",
        conversationId: "test-conv",
        conversationHistory: []
      };
      ...
      expect(planDescription.toLowerCase()).toMatch(/(blog|content|social|email|launch)/);
      
      // Should indicate next steps
      expect(planDescription.toLowerCase()).toMatch(/(review|approve|next|step)/);
    });
    ...
    it('should handle plan refinement intelligently', async () => {
      ...
      const initialResponse = await service.executeTask('executeTask', initialInput);
      expect(initialResponse.success).toBe(true);

      // Then refine it
      const refinementInput: OrchestratorInput = {
        prompt: "Actually, let's expand this to include social media promotion and email marketing to amplify the blog content",
        userId: "test-user",
        conversationId: "test-conv",
      ...
      const refinedResponse = await service.executeTask('executeTask', refinementInput);
      
      expect(refinedResponse.success).toBe(true);
      expect(refinedResponse.message || refinedResponse.response).toContain('social');
      expect(refinedResponse.message || refinedResponse.response).toContain('email');
    });
  });
  ...
  describe('LLM Conversation Context Intelligence', () => {
    ...
    it('should maintain context in marketing conversations', async () => {
      const conversationHistory = [
        ...
        },
        {
          ...
          content: 'I can help you develop a comprehensive marketing strategy. What specific aspects would you like to focus on?',
          timestamp: new Date().toISOString(),
          metadata: { agentName: 'marketing_manager_orchestrator' }
        }
      ];
      ...
      const input: OrchestratorInput = {
        prompt: "Let's focus on content marketing and thought leadership to establish our expertise in the AI space",
        userId: "test-user",
        conversationId: "test-conv",
        conversationHistory
      };
      ...
      expect(responseText.toLowerCase()).toMatch(/(content|thought leadership|ai|strategy)/);
    });
    ...
    it('should detect continuation with previously delegated agents', async () => {
      const conversationHistory = [
        ...
        {
          role: 'assistant' as const,
          content: 'I\'ve created a comprehensive blog post about AI in project management...',
          timestamp: new Date().toISOString(),
      ...
      const input: OrchestratorInput = {
        prompt: "That's great! Can you also create a shorter version for LinkedIn and Twitter?",
        userId: "test-user",
        conversationId: "test-conv",
        conversationHistory
      };
  ...
  describe('LLM Decision Quality Validation', () => {
    ...
    it('should demonstrate consistent high-quality marketing decisions', async () => {
      const testScenarios = [
        {
          prompt: "Write a technical blog post about machine learning algorithms",
          expectedDecision: 'delegate_to_blog_writer',
          description: 'Technical long-form content'
        },
        {
          prompt: "Create social media posts announcing our new product features",
        ...
        {
          prompt: "Plan a complete rebranding campaign with new messaging across all channels",
          expectedDecision: 'create_project',
          description: 'Complex multi-step initiative'
        },
        {
          prompt: "What's the best way to position our product against competitors?",
      ...
      for (const scenario of testScenarios) {
        ...
        try {
          ...
          if (response.projectId || response.planId) {
            ...
            const agent = response.agentName || response.metadata?.delegatedAgent;
            if (agent.includes('blog')) actualDecision = 'delegate_to_blog_writer';
            else if (agent.includes('content')) actualDecision = 'delegate_to_content_writer';
            else if (agent.includes('competitors')) actualDecision = 'delegate_to_competitors_agent';
            else actualDecision = 'delegate_to_other';
          } else {
            actualDecision = 'conversation';
          }

          const correct = actualDecision === scenario.expectedDecision || 
                         (actualDecision.startsWith('delegate') && scenario.expectedDecision.startsWith('delegate'));
      ...
      results.forEach(result => {
        console.log(`${result.correct ? '✅' : '❌'} ${result.description}: ${result.expectedDecision} → ${result.actualDecision}`);
      });
    ...
    it('should provide high-quality, complete responses', async () => {
      const input: OrchestratorInput = {
        prompt: "I'm launching a new B2B SaaS product and need a complete marketing strategy. What should I consider?",
        userId: "test-user",
        conversationId: "test-conv",
        conversationHistory: []
      };
      ...
      expect(responseText.toLowerCase()).toMatch(/(strategy|marketing|b2b|saas|target|audience|content|campaign)/);
      
      // Should include actionable suggestions
      expect(responseText.toLowerCase()).toMatch(/(recommend|suggest|consider|start|create|develop)/);
