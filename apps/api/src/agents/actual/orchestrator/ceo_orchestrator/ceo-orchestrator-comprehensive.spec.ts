...
describe('CEO Orchestrator - Comprehensive Cross-Departmental Intelligence Tests', () => {
  ...
  beforeEach(async () => {
    ...
    ceoOrchestrator = module.get<CEOOrchestratorService>(
      CEOOrchestratorService,
    );
  ...
  describe('Cross-Departmental Delegation Intelligence', () => {
    ...
    it('should intelligently delegate marketing strategy to marketing_manager_orchestrator', async () => {
      ...
      const marketingRequest = {
        prompt:
          'Develop a comprehensive go-to-market strategy for our new AI product. We need market positioning, competitive analysis, content strategy, and launch campaign coordination.',
      ...
      if (result.action === 'CLARIFY') {
        console.log(
          `✅ CEO triggered intelligent clarification for complex marketing strategy`,
        );
        ...
        ]).toContain(result.options?.delegate?.agentName);
      } else {
    ...
    it('should intelligently delegate technical strategy to engineering agents', async () => {
      ...
      const engineeringRequest = {
        prompt:
          'We need to define technical requirements for a new AI-powered project management system. Include architecture decisions, technology stack recommendations, and development roadmap.',
      ...
      console.log(
        `📊 Technical Strategy: ${result.response?.substring(0, 200)}...`,
      );

      // Should delegate to engineering agents for technical strategy
      expect(['requirements_writer', 'launcher', 'golf_rules_agent']).toContain(
        result.agentName,
      );
    }, 90000);
    ...
    it('should intelligently delegate HR initiatives to hr agents', async () => {
      ...
      const hrRequest = {
        prompt:
          'Design a comprehensive onboarding program for new engineering hires. Include technical training, culture integration, and performance milestone tracking.',
        userId: 'test-ceo-hr',
    ...
    it('should intelligently delegate financial planning to finance agents', async () => {
      ...
      const financeRequest = {
        prompt:
          'Analyze our current business metrics and create a financial forecast for Q2. Include revenue projections, cost analysis, and investment recommendations.',
      ...
      console.log(`✅ Delegated to: ${result.agentName}`);
      console.log(
        `📊 Financial Analysis: ${result.response?.substring(0, 200)}...`,
      );
    ...
    it('should intelligently delegate operational efficiency to operations agents', async () => {
      ...
      const operationsRequest = {
        prompt:
          'Optimize our team meeting processes and calendar management. Create standard operating procedures that improve productivity and reduce meeting overhead.',
      ...
      console.log(`✅ Delegated to: ${result.agentName}`);
      console.log(
        `📊 Operations Strategy: ${result.response?.substring(0, 200)}...`,
      );

      // Should delegate to operations/productivity agents
      expect(['calendar', 'meetings', 'sop', 'notion']).toContain(
        result.agentName,
      );
    }, 90000);
  });
  ...
  describe('Cross-Functional Project Coordination Intelligence', () => {
    ...
    it('should handle complex cross-departmental initiatives with clarification workflow', async () => {
      ...
      const complexRequest = {
        prompt:
          'Launch a company-wide digital transformation initiative that includes new technology implementation, employee training, process optimization, and performance tracking across all departments.',
        userId: 'test-ceo-complex',
        conversationId: 'test-conv-ceo-complex',
        conversationHistory: [],
      };

      const clarificationResult = await ceoOrchestrator.executeTask(
        'executeTask',
        complexRequest,
      );
      ...
      if (clarificationResult.action === 'CLARIFY') {
        console.log(
          `✅ Clarification triggered for cross-functional initiative`,
        );
        ...
        const delegationResult = await ceoOrchestrator.executeTask(
          'executeTask',
          delegationChoice,
        );
        ...
        console.log(
          `✅ Cross-functional delegation to: ${delegationResult.agentName}`,
        );
      } else {
        // Direct delegation or project creation
        console.log(
          `ℹ️ Direct handling: ${clarificationResult.action} → ${clarificationResult.agentName}`,
        );
    ...
    it('should demonstrate strategic decision-making for department coordination', async () => {
      ...
      const strategicRequests = [
        {
          prompt:
            "Our customer acquisition costs are too high. What's our strategy to improve this?",
          expectedDepartments: [
            'marketing_manager_orchestrator',
            'metrics',
            'chat_support',
        ...
        {
          prompt:
            'We need to improve employee productivity and satisfaction while reducing operational costs.',
          expectedDepartments: ['hr_assistant', 'calendar', 'sop', 'notion'],
          description: 'Employee productivity optimization',
        },
        {
          prompt:
            'Plan a new product feature that requires market research, technical development, and go-to-market strategy.',
          expectedDepartments: [
            'marketing_manager_orchestrator',
            'requirements_writer',
            'product_launch_coordinator',
      ...
      for (let i = 0; i < strategicRequests.length; i++) {
        ...
        if (result.action === 'CLARIFY') {
          ...
          const delegatedCorrectly = request.expectedDepartments.some(
            (dept) =>
              result.agentName?.includes(dept) ||
              dept.includes(result.agentName || ''),
          );
  ...
  describe('Executive Leadership Intelligence', () => {
    ...
    it('should handle executive-level strategic conversations directly', async () => {
      ...
      const executiveRequests = [
        {
          prompt:
            "What's our current competitive position and how should we respond to new market entrants?",
          description: 'Competitive strategy discussion',
        },
        {
          prompt:
            'How should we prioritize our Q2 initiatives given limited resources?',
          description: 'Resource allocation strategy',
        },
        {
          prompt:
            'What are the key risks facing our business and how do we mitigate them?',
      ...
      for (let i = 0; i < executiveRequests.length; i++) {
        ...
        console.log(
          `✅ Executive response: ${result.response?.substring(0, 100)}...`,
        );
    ...
    it('should demonstrate crisis management and escalation intelligence', async () => {
      ...
      const crisisRequest = {
        prompt:
          "We've discovered a critical security vulnerability in our product that affects all customers. We need immediate response coordination across engineering, customer support, legal, and communications.",
      ...
      console.log(
        `✅ Crisis response: ${result.response?.substring(0, 200)}...`,
      );
