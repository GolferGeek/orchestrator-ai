...
export class AgentDiscoveryService {
  ...
  async analyzeRequestForAgentMatch(
    ...
    if (useLLMReasoning) {
      try {
        ...
        this.logger.warn('LLM-powered reasoning failed, falling back to rule-based:', error);
      }
    }

    // Fallback to rule-based analysis
    return this.analyzeWithRules(userMessage, availableAgents, conversationHistory);
  }
  ...
  private analyzeWithRules(
    ...
    const explicitAgentMatch = this.findExplicitAgentRequest(lowerMessage, availableAgents);
    ...
    const contentMatch = this.analyzeContentForAgentMatch(lowerMessage, availableAgents);
    ...
    const continuityMatch = this.checkAgentContinuity(conversationHistory, availableAgents);
    if (continuityMatch) {
      return {
        action: 'delegate',
        agent: continuityMatch,
        confidence: 0.8,
        reasoning: 'Following up with previous agent based on conversation context'
      };
    }
   ...
   */
  findAgentByName(agentName: string, availableAgents: AvailableAgent[]): AvailableAgent | null {
    return availableAgents.find(agent => {
      const agentNameLower = agent.name.toLowerCase().replace(/[_\s-]+/g, ' ');
  ...
  private findExplicitAgentRequest(lowerMessage: string, availableAgents: AvailableAgent[]): AvailableAgent | null {
    const patterns = [
  ...
  private analyzeContentForAgentMatch(
    ...
    const domainMappings = [
      {
        keywords: ['golf', 'rules of golf', 'penalty', 'usga', 'r&a', 'handicap', 'water hazard', 'unplayable', 'relief', 'bunker', 'green', 'fairway', 'tee box'],
        agentNames: ['golf', 'rules'],
        confidence: 0.9
      },
      {
        keywords: ['blog', 'write', 'article', 'content', 'post', 'copywriting', 'creative writing'],
        agentNames: ['blog', 'content', 'writer'],
        confidence: 0.85
      },
      {
        keywords: ['calendar', 'schedule', 'meeting', 'appointment', 'time', 'date'],
    ...
    for (const mapping of domainMappings) {
      const hasKeyword = mapping.keywords.some(keyword => lowerMessage.includes(keyword));
      if (hasKeyword) {
  ...
  private async enhanceWithLLMReasoning(
    ...
    try {
      ...
      const systemPrompt = this.buildLLMOrchestrationPrompt(agentList, historyContext);
      ...
      if (decision) {
        this.logger.log(`LLM decision: ${decision.action} (confidence: ${decision.confidence})`);
        return decision;
      }
  ...
  private buildLLMOrchestrationPrompt(agentList: string, historyContext: string): string {
...
- Be decisive but conservative - prefer clarification over wrong delegation`;
  ...
  private parseLLMDecision(
    ...
    try {
      ...
      if (!decision.action || !['delegate', 'respond_directly', 'clarify'].includes(decision.action)) {
        return null;
      }

      let agent: AvailableAgent | undefined;
      if (decision.action === 'delegate' && decision.agent) {
        const foundAgent = this.findAgentByName(decision.agent, availableAgents);
        if (!foundAgent) {
  ...
  private parseStructuredResponse(
    ...
    } else if (lowerResponse.includes('respond_directly') || lowerResponse.includes('respond directly')) {
      return {
        action: 'respond_directly',
        confidence: 0.8,
  ...
  private checkSpecialCases(searchNameLower: string, agentNameLower: string): boolean {
    ...
    return specialCases.some(sc => 
      searchNameLower.includes(sc.search) && agentNameLower.includes(sc.agent)
    );
  }
}