...
export class ConversationContextService {
  ...
  shouldContinueWithSameAgent(
    ...
    const topicContinuity = this.checkTopicContinuity(
      userMessage,
      lastAgentMessage,
      conversationHistory,
    );
    if (topicContinuity.isRelated && confidence < topicContinuity.confidence) {
      ...
      reason =
        topicContinuity.confidence > confidence * 0.8
          ? topicContinuity.reason
          : reason;
    }

    // Temporal proximity boost - recent interactions are more likely to continue
    const messagesSinceLastAgent = this.getMessagesSinceLastAgent(
      conversationHistory,
      lastAgentMessage,
    );
    ...
    return {
      shouldContinue,
      agentName: shouldContinue
        ? lastAgentMessage.metadata?.agentName
        : undefined,
  ...
  private checkTopicContinuity(
    ...
    if (lastAgentMessage.content) {
      const lastAgentWords = lastAgentMessage.content
        .toLowerCase()
        .split(/\s+/);
      const userWords = lowerMessage.split(/\s+/);
      const commonWords = userWords.filter(
        (word) => word.length > 3 && lastAgentWords.includes(word),
      );
    ...
    return {
      isRelated: false,
      confidence: 0,
      reason: 'No topic continuity detected',
    };
  }
  ...
  extractUserInfo(
    ...
    if (preferencesFromHistory.length > 0) {
      userInfo.preferences =
        preferencesFromHistory[preferencesFromHistory.length - 1];
    }
  ...
  analyzeAgentInteractions(
    ...
    const agentRelationships: Array<{
      fromAgent: string;
      toAgent: string;
      handoffReason?: string;
    }> = [];
    ...
    for (const message of conversationHistory) {
      if (message.role === 'assistant' && message.metadata?.agentName) {
        ...
        agentSequence.push({
          ...
          interactionType: message.metadata?.isDelegated
            ? 'delegated'
            : 'direct',
        });
        ...
        if (lastAgent && lastAgent !== agentName) {
          agentRelationships.push({
            ...
            toAgent: agentName,
            handoffReason:
              message.metadata?.delegationReason || 'Topic change detected',
          });
        }
        ...
        agentSpecializations[agentName]?.push(
          ...topicsInResponse.filter(
    ...
    const currentAgentContext = this.calculateCurrentAgentContext(
      conversationHistory,
      agentSequence,
    );
  ...
  private calculateCurrentAgentContext(
    ...
    const lastAgentMessage = [...conversationHistory]
      ...
      .find(
        (msg) =>
          msg.role === 'assistant' && msg.metadata?.agentName === agentName,
      );
    ...
    const messagesSince = this.getMessagesSinceLastAgent(
      conversationHistory,
      lastAgentMessage,
    );
    ...
    const consecutiveInteractions = this.getConsecutiveAgentInteractions(
      agentSequence,
      agentName || '',
    );
  ...
  generateContextHandoff(
    ...
    const fromAgentMessages = conversationHistory
      .filter(
        (msg) =>
          msg.role === 'assistant' && msg.metadata?.agentName === fromAgent,
      )
    ...
    const handoffReason = this.determineHandoffReason(
      fromAgent,
      toAgent,
      userMessage,
      conversationHistory,
    ...
    const handoffStrength = this.calculateHandoffStrength(
      fromAgent,
      toAgent,
      conversationHistory,
    );
  ...
  private determineHandoffReason(
    ...
    if (
      lowerMessage.includes(`talk to ${toAgent.toLowerCase()}`) ||
  ...
  private calculateHandoffStrength(
    ...
    const agentInteractions =
      this.analyzeAgentInteractions(conversationHistory);

    // If agents have worked together before, increase strength
    const hasWorkedTogether = agentInteractions.agentRelationships.some(
      (rel) =>
  ...
  private getAgentDomain(agentName: string): string {
    ...
    if (lowerName.includes('golf') || lowerName.includes('rules'))
      return 'golf';
    if (lowerName.includes('blog') || lowerName.includes('content'))
      return 'content';
    if (lowerName.includes('calendar') || lowerName.includes('meeting'))
      return 'calendar';
  ...
  private analyzeConversationPatterns(
    ...
    for (const message of conversationHistory) {
      ...
      if (message.role === 'user') {
        ...
        patterns.topics.push(
          ...topics.filter((topic) => !patterns.topics.includes(topic)),
        );
      }
    }
