...
export async function execute(
  ...
  const { userMessage, llmService, progressCallback, mcpService, metadata } =
    params;
  ...
  if (!mcpService) {
    ...
    progressCallback?.(
      ...
      0,
      'failed',
      `MCP Client service unavailable - Supabase connection not available`,
    );
    throw new Error(
      'MCP Client service is required for Metrics Agent. Supabase database access not available.',
    );
  }

  console.log(
    '✅ MCP Service is available, proceeding with database operations',
  );
  ...
  try {
    ...
    progressCallback?.(
      ...
      0,
      'in_progress',
      'Connecting to Supabase via MCP Client...',
    );
    ...
    const schemaResult = await mcpService.callTool(
      'supabase-mcp',
      'get-schema',
      {
        format: 'json',
    ...
    console.log(
      '📊 Full schema result (first 500 chars):',
    ...
    console.log(
      '🔍 DEBUG: schemaResult.data?.success =',
      schemaResult.data?.success,
    );
    console.log('🔍 DEBUG: schemaResult.error =', schemaResult.error);
    console.log(
      '🔍 DEBUG: schemaResult.data?.error =',
      schemaResult.data?.error,
    );

    if (!schemaResult.success) {
      const error =
        schemaResult.error || schemaResult.data?.error || 'Unknown error';
    ...
    console.log(
      '🔍 DEBUG: typeof schemaResult.data =',
      typeof schemaResult.data,
    );
    console.log(
      '🔍 DEBUG: schemaResult.data keys =',
      schemaResult.data ? Object.keys(schemaResult.data) : 'null',
    );
    ...
    console.log(
      '🔍 DEBUG: schemaData keys =',
      schemaData ? Object.keys(schemaData) : 'null',
    );
    ...
    if (missingTables.length > 0) {
      ...
      throw new Error(
        `Missing required KPI tables: ${missingTables.join(', ')}`,
      );
    }
    ...
    progressCallback?.(
      ...
      0,
      'completed',
      `Connected successfully - validated ${requiredTables.length} KPI tables`,
    );
...
`;
    ...
    const testSQLGeneration = await mcpService.callTool(
      'supabase-mcp',
      'generate-sql',
      {
    ...
    if (testSQLGeneration.success && testSQLGeneration.data) {
      const sqlData =
        testSQLGeneration.data.data?.data || testSQLGeneration.data.data;
      console.log(
        '✅ SQL Generation working - generated query for KPI metrics count',
      );
      console.log(
        '📄 Generated SQL sample:',
        sqlData
          ? JSON.stringify(sqlData).substring(0, 200) + '...'
          : 'No SQL data',
      );

      // Test SQL execution
      console.log('🔍 VALIDATING: Testing SQL execution...');
      const testSQLExecution = await mcpService.callTool(
        'supabase-mcp',
        'execute-sql',
        {
      ...
      if (testSQLExecution.success && testSQLExecution.data) {
        const execData =
          testSQLExecution.data.data?.data || testSQLExecution.data.data;
        console.log(
          '✅ SQL Execution working - got results from kpi_metrics table',
        );
        ...
        progressCallback?.(
          ...
          2,
          'completed',
          'SQL generation and execution validated',
        );
      } else {
        const error =
          testSQLExecution.error ||
          testSQLExecution.data?.error ||
      ...
      const error =
        testSQLGeneration.error ||
        testSQLGeneration.data?.error ||
    ...
    const sqlGenResult = await mcpService.callTool(
      'supabase-mcp',
      'generate-sql',
      {
        prompt: userMessage,
    ...
    if (sqlGenResult.success && sqlGenResult.data) {
      try {
        ...
        // Fallback for legacy format
          ...
          try {
            const parsed = JSON.parse(
              sqlGenResult.data.tool_result.content[0].text,
            );
        ...
        if (generatedSQL) {
          ...
          userQueryResult = await mcpService.callTool(
            'supabase-mcp',
            'execute-sql',
            {
              sql: generatedSQL,
          ...
          console.log(
            '📊 User query result:',
            userQueryResult.success ? 'SUCCESS' : 'FAILED',
          );
          progressCallback?.(
            'SQL Execution',
            2.7,
            userQueryResult.success ? 'completed' : 'failed',
            userQueryResult.success
              ? 'SQL executed successfully'
              : `SQL execution failed: ${userQueryResult.error || 'Unknown error'}`,
          );
        } else {
          progressCallback?.(
            'SQL Generation',
            2.5,
            'failed',
            'No SQL was generated from the user query',
          );
        }
      } catch (e) {
    ...
    const metricsResult = await mcpService.callTool(
      'supabase-mcp',
      'read-data',
      {
    ...
    console.log(
      '🔍 DEBUG: metricsResult FULL structure:',
    ...
    if (metricsResult.success && metricsResult.data) {
      ...
      console.log(
        '🔍 DEBUG: metricsResult.data keys:',
        Object.keys(metricsResult.data),
      );
      ...
      if (metricsResult.data.tool_result?.content?.[0]?.text) {
        ...
        try {
          const parsed = JSON.parse(
            metricsResult.data.tool_result.content[0].text,
          );
          console.log(
            '🔍 DEBUG: parsed tool_result FULL:',
        ...
        console.log(
          '🔍 DEBUG: Found data.data format:',
          JSON.stringify(metricsResult.data.data, null, 2),
        );
        ...
        if (Array.isArray(metricsResult.data.data)) {
          ...
          metricsResult.data.data.data &&
          Array.isArray(metricsResult.data.data.data)
        ) {
          metricsData = metricsResult.data.data.data;
          dataPath = 'data.data.data';
        } else if (
          metricsResult.data.data.rows &&
          Array.isArray(metricsResult.data.data.rows)
        ) {
      ...
      console.log(
        '📄 FIRST RECORD SAMPLE:',
        metricsData.length > 0 ? JSON.stringify(metricsData[0]) : 'No records',
      );
      ...
      progressCallback?.(
        ...
        2,
        'completed',
        `Retrieved ${metricsData.length} metric definitions from ${dataPath}`,
      );
    } else {
      const error =
        metricsResult.error || metricsResult.data?.error || 'Unknown error';
      ...
      progressCallback?.(
        ...
        2,
        'failed',
        `Failed to retrieve metrics: ${error}`,
      );
    }
    ...
    progressCallback?.(
      ...
      3,
      'in_progress',
      'Getting recent KPI summary via MCP Client...',
    );

    const kpiDataResult = await mcpService.callTool(
      'supabase-mcp',
      'read-data',
      {
        table_name: 'kpi_data',
    ...
    if (kpiDataResult.success && kpiDataResult.data) {
      ...
      const kpiData = Array.isArray(kpiDataResult.data.data?.data)
        ? kpiDataResult.data.data.data
        : [];
      ...
      progressCallback?.(
        ...
        3,
        'completed',
        `Retrieved ${dataCount} recent KPI data points`,
      );
    } else {
      const error =
        kpiDataResult.error || kpiDataResult.data?.error || 'Unknown error';
      progressCallback?.(
        'KPI data',
        3,
        'failed',
    ...
    progressCallback?.(
      ...
      4,
      'in_progress',
      'Getting KPI goals sample via MCP Client...',
    );
    ...
    if (goalsResult.success && goalsResult.data) {
      // Handle MCP Client wrapper: result.data contains the actual MCP server response
      const goalsData = Array.isArray(goalsResult.data.data?.data)
        ? goalsResult.data.data.data
        : [];
      ...
      progressCallback?.(
        ...
        4,
        'completed',
        `Retrieved ${goalsCount} KPI goals and targets`,
      );
    } else {
      const error =
        goalsResult.error || goalsResult.data?.error || 'Unknown error';
      progressCallback?.(
        'Goals data',
        4,
        'failed',
        `Failed to retrieve goals: ${error}`,
      );
    }
    ...
    progressCallback?.(
      ...
      5,
      'in_progress',
      'Generating insights from live database data...',
    );
    ...
    function extractDataFromMCPResult(result: any, resultName: string): any[] {
      ...
      if (resultName === 'USER_QUERY') {
        ...
        console.log(
          'First 200 chars of result.data:',
      ...
      if (resultName === 'USER_QUERY') {
        ...
        if (
          ...
          result.data.tool_result?.content?.[0]?.text
        ) {
          try {
            ...
            if (
              ...
              dataPath =
                'USER_QUERY: tool_result.content[0].text (direct array)';
            }
          } catch (e) {
            console.log(
              `⚠️ ${resultName}: Could not parse tool_result text:`,
              e,
            );
          }
        }
      ...
      console.log(
        `✅ ${resultName}: Found ${extractedData.length} records at path: ${dataPath}`,
      );
      return extractedData;
    }

    const metricsDefinitions = extractDataFromMCPResult(
      metricsResult,
      'METRICS_DEFINITIONS',
    );
    ...
    if (userQueryResult && userQueryResult.success && userQueryResult.data) {
      console.log(
        '🔍 DEBUG: userQueryResult structure:',
        JSON.stringify(userQueryResult, null, 2),
      );
      userQueryResults = extractDataFromMCPResult(
        userQueryResult,
        'USER_QUERY',
      );
      console.log('🔍 DEBUG: Extracted userQueryResults:', userQueryResults);
      console.log(
        '🔍 DEBUG: userQueryResults length:',
        userQueryResults ? userQueryResults.length : 'null',
      );
    } else {
      ...
      if (sqlGenResult.success && sqlGenResult.data) {
        ...
        if (typeof sqlGenResult.data === 'object') {
          generatedSQLForReport =
            sqlGenResult.data.sql ||
            sqlGenResult.data.query ||
        ...
        // Fallback for legacy format
          ...
          generatedSQLForReport =
            parsed.sql || parsed.query || 'No SQL found in response';
        }
      }
    } catch (e) {
...
`;
    ...
    const totalDataPoints =
      metricsDefinitions.length + currentKpiData.length + goalsData.length;
    progressCallback?.(
      'Analysis complete',
      6,
      'completed',
      `Generated metrics analysis from ${totalDataPoints} data points (${metricsDefinitions.length} definitions, ${currentKpiData.length} records, ${goalsData.length} goals)`,
    );
      ...
      metadata: {
        ...
        processingTime:
          ...
          (metadata?.timestamp
            ...
            : Date.now()),
        toolsUsed: [
          'MCP Client Service (Failed)',
          'Supabase MCP Server (Failed)',
        ],
        responseType: 'error',
        mcpEnabled: false,
