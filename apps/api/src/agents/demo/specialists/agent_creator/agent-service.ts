...
import {
  ...
  AgentSkillData,
  ConversationData,
} from './services/agent-configuration.service';
...
export class AgentCreatorService extends ContextAgentBaseService {
  ...
  private readonly STRUCTURED_QUESTIONS = [
    {
      ...
      field: 'agentName',
      question:
        'What would you like to name your agent? (This will be the unique identifier)',
      validation: (answer: string) => {
        if (!answer || answer.trim().length < 2)
          return 'Agent name must be at least 2 characters';
        if (!/^[a-zA-Z][a-zA-Z0-9_\s]*$/.test(answer))
          return 'Agent name must start with a letter and contain only letters, numbers, underscores, and spaces';
        return null;
      },
    },
    {
      ...
      field: 'description',
      question:
        'Provide a brief description of what this agent does (1-2 sentences)',
      validation: (answer: string) => {
    ...
    },
    {
      ...
      field: 'department',
      question:
        'Which department should this agent belong to? (marketing, engineering, operations, finance, hr, sales, research, product, specialists)',
      validation: (answer: string) => {
        const validDepts = [
          'marketing',
          'engineering',
        ...
        if (!validDepts.includes(normalized))
    ...
    },
    {
      ...
      question:
        'What is the primary purpose or problem this agent solves? (Be specific)',
      validation: (answer: string) => {
        if (!answer || answer.trim().length < 20)
          return 'Primary purpose must be at least 20 characters';
        if (answer.trim().length > 1000)
          return 'Primary purpose must be less than 1000 characters';
        return null;
      },
    },
    {
    ...
    },
    {
      ...
      field: 'skillName',
      question:
        'What is the primary skill this agent provides? (e.g., "Content Writing", "Data Analysis")',
      validation: (answer: string) => {
    ...
    },
    {
      ...
      question:
        'Describe this skill in detail (what exactly can the agent do?)',
      validation: (answer: string) => {
        if (!answer || answer.trim().length < 10)
          return 'Skill description must be at least 10 characters';
        if (answer.trim().length > 500)
          return 'Skill description must be less than 500 characters';
        return null;
      },
    },
    {
      ...
      field: 'skillExamples',
      question:
        'Provide 3-5 specific examples of tasks this agent can handle (one per line)',
      validation: (answer: string) => {
        const examples = answer
          .split('\n')
          .map((e) => e.trim())
          .filter((e) => e);
        if (examples.length < 3) return 'At least 3 examples are required';
        if (examples.length > 10) return 'Maximum 10 examples allowed';
        if (examples.some((e) => e.length < 5))
          return 'Each example must be at least 5 characters';
        return null;
      },
    },
    {
      ...
      question:
        'What communication style should this agent use? (professional, casual, technical, conversational, formal, creative)',
      validation: (answer: string) => {
        const validStyles = [
          'professional',
          'casual',
        ...
        if (!validStyles.includes(normalized))
          return `Communication style must be one of: ${validStyles.join(', ')}`;
        return null;
      },
    },
    {
      ...
      field: 'limitations',
      question:
        'What are the limitations or things this agent should NOT do? (comma-separated)',
      validation: (answer: string) => {
    ...
    },
    {
      ...
      field: 'coreIdentity',
      question:
        'How should this agent present itself to users? (Write a brief identity statement)',
      validation: (answer: string) => {
        if (!answer || answer.trim().length < 20)
          return 'Core identity must be at least 20 characters';
        if (answer.trim().length > 300)
          return 'Core identity must be less than 300 characters';
        return null;
      },
    },
  ];
  ...
  async processTask(taskRequest: any): Promise<any> {
    try {
      this.logger.debug(
        `Agent Creator processing task. User message: ${taskRequest.prompt?.substring(0, 100)}...`,
      );
      ...
      const conversation =
        await this.agentConfigService.getConversation(sessionId);
      ...
      return await this.continueStructuredConversation(
        conversation,
        userMessage,
        userId,
      );
   ...
   */
  private async continueStructuredConversation(
    conversation: ConversationData,
    userMessage: string,
    userId?: string,
  ): Promise<any> {
    const currentQuestion =
      this.STRUCTURED_QUESTIONS[conversation.currentQuestion - 1];

    if (!currentQuestion) {
      this.logger.error(
        `Invalid question number: ${conversation.currentQuestion}`,
      );
    ...
    const updatedRequirements = {
      ...
      [currentQuestion.field]: this.normalizeAnswer(
    ...
    const completionPercentage = Math.round(
      (conversation.currentQuestion / this.STRUCTURED_QUESTIONS.length) * 100,
    );
    ...
    if (conversation.currentQuestion >= this.STRUCTURED_QUESTIONS.length) {
      ...
      return await this.createAgentFromRequirements(
        updatedRequirements,
        userId,
      );
    }
   ...
   */
  private async createAgentFromRequirements(
    requirements: any,
    userId?: string,
  ): Promise<any> {
    try {
      ...
      const agentData: AgentConfigurationData = {
        ...
        displayName: requirements.agentName
          .replace(/_/g, ' ')
      ...
      const skillData: AgentSkillData = {
        skillId: requirements.skillName
          .toLowerCase()
          .replace(/[^a-z0-9]/g, '_'),
...
- Be helpful while respecting your limitations`;
  ...
  private generateServiceContent(requirements: any): string {
    const className = requirements.agentName
      .split('_')
...
export class ${className}Service extends ContextAgentBaseService {
  ...
  private async cleanupOldConversations(): Promise<void> {
    ...
    this.logger.debug(
      'Conversation cleanup should be implemented as a scheduled job',
    );
  }
}
