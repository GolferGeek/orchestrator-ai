...
import { UserCreateDto, UserLoginDto, TokenResponseDto, AuthenticatedUserResponseDto, SupabaseAuthUserDto } from './dto/auth.dto';
...
export class AuthService {
  ...
  async signup(userCreateDto: UserCreateDto): Promise<TokenResponseDto> {
    ...
    try {
      ...
      const { data: authResponse, error } = await supabaseClient.auth.signUp({
        ...
        options: {
          data: {
            display_name: userCreateDto.display_name || userCreateDto.email.split('@')[0]
          }
        }
      });
      ...
      if (error) {
        ...
        throw new BadRequestException(error.message || 'Error during signup. User might already exist or invalid input.');
      }

      // Handle successful signup with session
      if (authResponse.user && authResponse.session?.access_token) {
        this.logger.log(`User created successfully with session: ${authResponse.user.id}`);
        return {
      ...
      if (authResponse.user && !authResponse.session) {
        this.logger.log(`User created but email confirmation required: ${authResponse.user.id}`);
      ...
      // Unexpected response
      throw new BadRequestException('Could not create user or establish session.');
  ...
  async login(userLoginDto: UserLoginDto): Promise<TokenResponseDto> {
    ...
    try {
      ...
      const { data: authResponse, error } = await supabaseClient.auth.signInWithPassword({
      ...
      if (!authResponse.session?.access_token) {
        this.logger.error(`Login succeeded but no session or token received for: ${userLoginDto.email}`);
        throw new BadRequestException('Login succeeded but no session or token received.');
      }
  ...
  async getCurrentUser(currentAuthUser: SupabaseAuthUserDto, token: string): Promise<AuthenticatedUserResponseDto> {
    ...
    try {
      ...
      if (userData) {
        ...
        this.logger.warn(`No public user profile found for auth user: ${currentAuthUser.id}`);
        return {
          id: currentAuthUser.id,
      ...
      this.logger.error(`Error fetching user profile for ${currentAuthUser.id}: ${error}`);
      throw new HttpException(
