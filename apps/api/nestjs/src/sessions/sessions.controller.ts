...
export class SessionsController {
  ...
  async createSession(
    ...
    return this.sessionsService.createSession(sessionCreateDto, currentUser, token);
  }
  ...
  @ApiQuery({ name: 'skip', required: false, description: 'Number of sessions to skip for pagination', example: 0 })
  @ApiQuery({ name: 'limit', required: false, description: 'Maximum number of sessions to return', example: 100 })
  async listSessions(
    @CurrentUser() currentUser: SupabaseAuthUserDto,
    @Request() req: any,
    ...
    return this.sessionsService.listSessions(currentUser, token, Number(skip), Number(limit));
  }
  ...
  @ApiParam({ name: 'sessionId', description: 'Session ID', format: 'uuid' })
  @ApiQuery({ name: 'skip', required: false, description: 'Number of messages to skip for pagination', example: 0 })
  ...
  async getSessionMessages(
    ...
    return this.sessionsService.getSessionMessages(sessionId, currentUser, token, Number(skip), Number(limit));
  }
