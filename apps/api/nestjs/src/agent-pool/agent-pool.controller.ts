import { Controller, Get, Post, Body, Param, Delete, HttpCode, Logger } from '@nestjs/common';
import { ApiTags, ApiOperation, ApiResponse, ApiBody } from '@nestjs/swagger';
import { AgentPoolService } from './agent-pool.service';
import { AgentRegistration, AgentHeartbeat, AgentCapabilitiesDocument, PoolStats } from './interfaces';
...
export class AgentPoolController {
  ...
  async receiveHeartbeat(@Body() heartbeat: AgentHeartbeat) {
    try {
      ...
      this.logger.error(`Error processing heartbeat from ${heartbeat.agentId}:`, error);
      throw error;
    }
  }
  ...
  @Get('capabilities')
  @ApiOperation({ summary: 'Get comprehensive capabilities document for orchestrator' })
  @ApiResponse({ 
    status: 200, 
  ...
  @ApiOperation({ summary: 'Get orchestrator-friendly agent list for LLM prompts' })
  @ApiResponse({ 
    status: 200, 
    ...
    schema: {
      type: 'object',
      properties: {
        agentList: { type: 'string', description: 'Formatted agent capabilities for LLM prompts' },
