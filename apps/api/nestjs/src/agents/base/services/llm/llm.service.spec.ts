...
import { LangSmithIntegrationService } from './langsmith-integration.service';

describe('LLMService', () => {
  let service: LLMService;
  let langSmithService: LangSmithIntegrationService;
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        LangSmithIntegrationService,
        LLMService,
      ],
    }).compile();

    service = module.get<LLMService>(LLMService);
    langSmithService = module.get<LangSmithIntegrationService>(LangSmithIntegrationService);
  });
  ...
  describe('LangSmith Integration', () => {
    it('should integrate with LangSmith service for LLM decoration', () => {
      const llm = service.getLangGraphLLM('openai');
      
      // Verify that the LLM has been decorated by LangSmith
      // The decoration should be transparent, so we just check it returns an LLM
      expect(llm).toBeDefined();
      expect(typeof llm.invoke).toBe('function');
    });

    it('should handle LangSmith integration errors gracefully', () => {
      // Test that service still works even if LangSmith integration fails
      expect(() => service.getLangGraphLLM('openai')).not.toThrow();
    });

    it('should pass through metadata to LangSmith decoration', () => {
      const llm = service.getLangGraphLLM('anthropic');
      expect(llm).toBeDefined();
      
      // The LangSmith service should have been called with provider metadata
      // We can't easily test the internal call, but we can verify the LLM works
      expect(typeof llm.invoke).toBe('function');
    });
  });
}); 