...
export abstract class A2AAgentBaseService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    ...
    try {
      ...
      this.loggingService.logAgentEvent(agentName, 'initialized', { agentPath: this.agentPath });
    } catch (error) {
      this.loggingService.logError(error instanceof Error ? error : new Error(String(error)), {
        agentName,
  ...
  async processJsonRpcRequest(request: JsonRpcRequest): Promise<JsonRpcResponse> {
    const startTime = Date.now();
    const authContext = this.authService.extractAuthContext(request.params || {});
    ...
    try {
      ...
      const methodHandler = async (method: string, params: any): Promise<any> => {
        return this.executeTask(method, params);
      };

      // Create a notification handler (not used but required by interface)
      const notificationHandler = async (notification: JsonRpcNotification): Promise<void> => {
        this.loggingService.logAgentEvent(this.getAgentName(), 'notification_received', {
      ...
      if (response === null) {
        ...
        this.loggingService.logResponse(request.method, true, responseTime, logContext);
        return this.jsonRpcProtocolService.createSuccessResponse(request.id || null, null);
      } else if (Array.isArray(response)) {
        this.loggingService.logResponse(request.method, true, responseTime, logContext);
        return response[0] || this.jsonRpcProtocolService.createErrorResponse(
        ...
        this.loggingService.logResponse(request.method, !hasError, responseTime, logContext);
        return response;
      } else {
        // Fallback for undefined response
        this.loggingService.logResponse(request.method, false, responseTime, logContext);
      ...
      const responseTime = Date.now() - startTime;
      this.loggingService.logError(error instanceof Error ? error : new Error(String(error)), logContext);
      this.loggingService.logResponse(request.method, false, responseTime, logContext);
  ...
  async processTask(taskRequest: any): Promise<any> {
    ...
    if (taskRequest && taskRequest.jsonrpc === '2.0' && taskRequest.method && taskRequest.id) {
  ...
  getAgentName(): string {
    ...
    return className.replace('Service', '').replace(/([A-Z])/g, ' $1').trim();
  }
  ...
  private async registerWithAgentPool(): Promise<void> {
    try {
      ...
      this.logger.error(`Failed to register agent ${this.getAgentName()}:`, error);
      throw error;
    }
  }
      ...
      for (const line of stackLines) {
        if (line.includes('agents/actual/') && line.includes('agent-service')) {
          ...
          if (match && match[1]) {
            ...
            const cleanPath = fullPath.replace(/\/agent-service\.(ts|js).*$/, '');
            return cleanPath;
          }
        }
      }
  ...
  private buildAgentUrl(): string {
    ...
    console.log(`[DEBUG] buildAgentUrl: baseUrl=${baseUrl}, agentType=${agentType}, agentName=${agentName}, final URL=${url}`);
    return url;
  }
} 