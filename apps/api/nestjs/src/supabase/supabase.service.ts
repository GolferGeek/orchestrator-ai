...
export class SupabaseService implements OnModuleInit {
  ...
  private initializeClients() {
    ...
    if (anonKey) {
      try {
        this.logger.log(`Attempting to create Supabase anon client for URL: ${url.substring(0, 20)}...`);
        this.anonClient = createClient(url, anonKey);
        this.logger.log('Supabase anon client created successfully');
      } catch (error) {
        const errorMessage = error instanceof Error ? error.message : 'Unknown error';
        const errorStack = error instanceof Error ? error.stack : undefined;
        this.logger.error(`Error creating Supabase anon client: ${errorMessage}`, errorStack);
        throw error;
      }
    } else {
      this.logger.warn('SUPABASE_ANON_KEY not configured. Anonymous client not available.');
    }

    // Initialize service client (bypasses RLS - use with caution)
    if (serviceKey) {
      try {
        this.logger.log(`Attempting to create Supabase service client for URL: ${url.substring(0, 20)}...`);
        this.serviceClient = createClient(url, serviceKey);
        this.logger.log('Supabase service client created successfully');
      } catch (error) {
        const errorMessage = error instanceof Error ? error.message : 'Unknown error';
        const errorStack = error instanceof Error ? error.stack : undefined;
        this.logger.error(`Error creating Supabase service client: ${errorMessage}`, errorStack);
        throw error;
      }
    } else {
      this.logger.warn('SUPABASE_SERVICE_ROLE_KEY not configured. Service client not available.');
    }
  }
  ...
  getAnonClient(): SupabaseClient {
    if (!this.anonClient) {
      this.logger.error('Supabase anonymous client is not available. Check configuration.');
  ...
  getServiceClient(): SupabaseClient {
    if (!this.serviceClient) {
      this.logger.error('Supabase service client is not available. Check configuration.');
      throw new HttpException(
  ...
  createAuthenticatedClient(token: string): SupabaseClient {
    ...
    if (!url || !anonKey) {
      this.logger.error('Supabase URL or Anon Key not configured for authenticated client creation');
      throw new HttpException(
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      const errorStack = error instanceof Error ? error.stack : undefined;
      this.logger.error(`Error creating authenticated Supabase client: ${errorMessage}`, errorStack);
      throw new HttpException(
  ...
  async executeQuery<T>(
    ...
    const client = useServiceClient ? this.getServiceClient() : this.getAnonClient();
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      const errorStack = error instanceof Error ? error.stack : undefined;
      this.logger.error(`Database operation failed: ${errorMessage}`, errorStack);
      throw error;
    }
  }
