...
export class AppService implements OnModuleInit {
  ...
  constructor(
    ...
    this.logger.log(
      '🔥 AppService constructor - LLMService injected successfully!',
    );
  ...
  async onModuleInit() {
    ...
    const discoveredAgents =
      await this.agentDiscovery.discoverAndInstantiateAgents();
    const agentInstances = this.agentDiscovery.getAgentInstances();

    if (discoveredAgents.length > 0) {
      this.logger.log(
        `✅ Discovered and instantiated ${discoveredAgents.length} agents:`,
      );
      discoveredAgents.forEach((agent) => {
        this.logger.log(
          `   • ${agent.type}: ${agent.name} (${agent.serviceClass?.name || 'Unknown'})`,
        );
      });
      this.logger.log(
        `🚀 ${agentInstances.length} agent services are running and auto-registered`,
      );
    } else {
