...
@router.delete("/{session_id}", summary="Delete a specific chat session", status_code=status.HTTP_204_NO_CONTENT)
async def delete_session(
    ...
    try:
        ...
        if not session_check.data:
            logger.warning(f"User {current_user.id} attempted to delete session {session_id} they don't own or doesn't exist.")
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Session not found or access denied.")
        
        # Delete the session (messages will be deleted automatically via cascade)
        delete_response = (
            supabase_client.table("sessions")
            .delete()
            .eq("id", str(session_id))
        ...
        # No need to check response data for a successful delete operation
        logger.info(f"Session {session_id} successfully deleted by user {current_user.id}")
        return None
