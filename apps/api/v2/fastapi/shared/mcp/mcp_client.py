...
from apps.api.v2.fastapi.core.config import Settings, settings as global_settings # Import Settings and global instance
...
class MCPClient:
    ...
    def __init__(self, http_client: httpx.AsyncClient):
        ...
        # Get API_BASE_URL from environment or default
        self.api_base_url = os.getenv("API_BASE_URL")
        if not self.api_base_url:
            self.api_base_url = "http://localhost:8000" # Default for local development
            logger.warning(
                f"MCPClient: API_BASE_URL env var not set. Defaulting to {self.api_base_url}."
            )
        self.timeout = 30.0  # Default timeout for MCP requests
    ...
    async def query_agent_stream(
        ...
        """
        url_path = f"/mcp/stream/{agent_id}" # Path relative to API base
        mcp_agent_stream_url = f"{self.api_base_url.strip('/')}{url_path}" # Construct full URL
        self.logger.info(f"MCPClient: Attempting to stream from {mcp_agent_stream_url}")

        payload = {"user_query": user_query, "agent_id": agent_id} # agent_id in payload might be redundant if in URL
        if session_id:
