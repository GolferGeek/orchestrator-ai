...
import asyncio # Added for awaiting coroutines

# from apps.api.v2.fastapi.main import app # No longer need the global app instance here
from apps.api.v2.fastapi.agents.system.orchestrator.service import OrchestratorAgentService 
# AGENT_VERSION will now come from the service class if needed, e.g., OrchestratorAgentService.agent_version

from apps.api.v2.fastapi.a2a_protocol.task_store import TaskStoreService
from apps.api.v2.fastapi.a2a_protocol.types import Message, TextPart, TaskSendParams, AgentCard, Task, TaskState, TaskStatus, TaskAndHistory
from apps.api.v2.fastapi.llm.openai_service import OpenAIService # For type hinting the mock
...
# Use attributes from the new service class for expected values
EXPECTED_ORCHESTRATOR_AGENT_ID = OrchestratorAgentService.agent_id
EXPECTED_ORCHESTRATOR_AGENT_NAME = OrchestratorAgentService.display_name # display_name for user-facing name
EXPECTED_AGENT_VERSION = OrchestratorAgentService.agent_version # Get version from the service
...
# Helper function to await any unawaited coroutines from an AsyncMock
async def await_unawaited_coroutines(mock: AsyncMock):
    for _, args, kwargs in mock.mock_calls:
        # Handle any coroutine objects that might have been created but not awaited
...
async def test_orchestrator_get_agent_card(client_and_app: tuple[httpx.AsyncClient, FastAPI]):
    ...
    response = await client.get(f"/agents/{OrchestratorAgentService.department_name}/{OrchestratorAgentService.agent_name}/agent-card")
    assert response.status_code == 200
    agent_card = response.json()
    assert agent_card["id"] == EXPECTED_ORCHESTRATOR_AGENT_ID
    assert agent_card["name"] == EXPECTED_ORCHESTRATOR_AGENT_NAME
    assert agent_card["version"] == EXPECTED_AGENT_VERSION
    assert f"/agents/{OrchestratorAgentService.department_name}/{OrchestratorAgentService.agent_name}/tasks" in agent_card["endpoints"]
    ...
    assert "natural_language_understanding" in capability_names
    assert "agent_delegation" in capability_names
...
async def test_orchestrator_respond_directly(client_and_app: tuple[httpx.AsyncClient, FastAPI], mock_openai_service: AsyncMock, mocker: AsyncMock):
    ...
    task_params = create_simple_task_send_params("What is a tomato?")
    response = await client.post(f"/agents/{OrchestratorAgentService.department_name}/{OrchestratorAgentService.agent_name}/tasks", json=task_params.model_dump(mode='json'))
    assert response.status_code == 200
    response_data = response.json()
    assert response_data["response_message"]["parts"][0]["text"] == expected_llm_response
    
    # Make sure all mock calls are awaited
    assert mock_openai_service.decide_orchestration_action.await_count == 1
    
    # Explicitly await any unawaited coroutines
    await await_unawaited_coroutines(mock_openai_service.decide_orchestration_action)
    
    # Reset at end of test to clear any unawaited coroutines
...
async def test_orchestrator_delegate_to_metrics(
    ...
    task_params = create_simple_task_send_params("What are the current sales figures?")
    response = await client.post(f"/agents/{OrchestratorAgentService.department_name}/{OrchestratorAgentService.agent_name}/tasks", json=task_params.model_dump(mode='json'))
...
async def test_orchestrator_clarify(client_and_app: tuple[httpx.AsyncClient, FastAPI], mock_openai_service: AsyncMock, mocker: AsyncMock):
    ...
    task_params = create_simple_task_send_params("Tell me about stuff.")
    response = await client.post(f"/agents/{OrchestratorAgentService.department_name}/{OrchestratorAgentService.agent_name}/tasks", json=task_params.model_dump(mode='json'))
    assert response.status_code == 200
    response_data = response.json()
...
async def test_orchestrator_cannot_handle(client_and_app: tuple[httpx.AsyncClient, FastAPI], mock_openai_service: AsyncMock, mocker: AsyncMock):
    ...
    task_params = create_simple_task_send_params("Solve world hunger.")
    response = await client.post(f"/agents/{OrchestratorAgentService.department_name}/{OrchestratorAgentService.agent_name}/tasks", json=task_params.model_dump(mode='json'))
    assert response.status_code == 200
    response_data = response.json()
...
async def test_orchestrator_unknown_llm_action(client_and_app: tuple[httpx.AsyncClient, FastAPI], mock_openai_service: AsyncMock, mocker: AsyncMock):
    ...
    task_params = create_simple_task_send_params("What if the LLM returns a weird action?")
    response = await client.post(f"/agents/{OrchestratorAgentService.department_name}/{OrchestratorAgentService.agent_name}/tasks", json=task_params.model_dump(mode='json'))
    assert response.status_code == 200
    response_data = response.json()
    expected_text = "I'm not sure how to proceed based on the information received. (Action: do_magic)"
    assert response_data["response_message"]["parts"][0]["text"] == expected_text
    # Make sure all mock calls are awaited
    assert mock_openai_service.decide_orchestration_action.await_count == 1
...
async def test_orchestrator_llm_call_exception(
    ...
    task_params = create_simple_task_send_params("Analyze this complex dataset.")
    response = await client.post(f"/agents/{OrchestratorAgentService.department_name}/{OrchestratorAgentService.agent_name}/tasks", json=task_params.model_dump(mode='json'))
    assert response.status_code == 200 
    response_data = response.json()
    expected_text = f"I encountered an error trying to understand your request: {llm_error_message}"
    assert response_data["response_message"]["parts"][0]["text"] == expected_text
    # Make sure all mock calls are awaited
    assert mock_openai_service.decide_orchestration_action.await_count == 1
...
async def test_orchestrator_openai_service_not_available(
    ...
    task_params = create_simple_task_send_params("Summarize this document for me.")
    response = await client.post(f"/agents/{OrchestratorAgentService.department_name}/{OrchestratorAgentService.agent_name}/tasks", json=task_params.model_dump(mode='json'))
    assert response.status_code == 200
    response_data = response.json()
    
    # Corrected: Orchestrator falls back to a simpler message when LLM decision is None
    expected_text = "I encountered an issue trying to process your request due to an invalid routing decision."
    assert response_data["response_message"]["parts"][0]["text"] == expected_text
    # Make sure all mock calls are awaited
    assert mock_openai_service.decide_orchestration_action.await_count == 1
...
async def test_orchestrator_cancel_task_not_found(client_and_app: tuple[httpx.AsyncClient, FastAPI], mocker: AsyncMock):
    ...
    response = await client.delete(f"/agents/{OrchestratorAgentService.department_name}/{OrchestratorAgentService.agent_name}/tasks/{task_id}")
...
async def test_orchestrator_cancel_active_task(client_and_app: tuple[httpx.AsyncClient, FastAPI], mocker: AsyncMock):
    ...
    cancelled_status_text = "Task cancellation processed."
    ...
    assert response_data["id"] == task_id
    assert response_data["status"] == "cancelled" 
    assert response_data["message"] == cancelled_status_text 
