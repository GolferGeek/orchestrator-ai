...
from .schemas import SupabaseAuthUser
...
async def get_supabase_client_as_current_user(
    ...
    try:
        ...
        if not user_response or not user_response.user:
            logger.warning(f"get_supabase_client_as_current_user: No user found for token on new client. Token: {token[:20] if token else 'None'}...")
            raise credentials_exception
        ...
        logger.warning(f"get_supabase_client_as_current_user: Supabase auth error on new client: {e}. Token: {token[:20] if token else 'None'}...")
        raise credentials_exception
    except Exception as e:
        logger.error(f"get_supabase_client_as_current_user: Unexpected error on new client: {e}. Token: {token[:20] if token else 'None'}...", exc_info=True)
        raise credentials_exception
