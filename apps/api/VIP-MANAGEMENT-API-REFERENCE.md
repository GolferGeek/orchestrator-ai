...
**Base URL**: `http://localhost:9000/sanitization`
...
```bash
curl -X GET "http://localhost:9000/sanitization/pii/patterns"
```
**Response:**
```json
{
...
```bash
curl -X POST "http://localhost:9000/sanitization/pii/patterns" \
...
```bash
curl -X PUT "http://localhost:9000/sanitization/pii/patterns/vip_tech_executives" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X DELETE "http://localhost:9000/sanitization/pii/patterns/vip_tech_executives"
```
**Response:**
```json
{
...
```bash
curl -X POST "http://localhost:9000/sanitization/pii/test" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X GET "http://localhost:9000/sanitization/redaction/patterns"
```

### **POST** `/redaction/patterns` - Add Redaction Pattern
```bash
curl -X POST "http://localhost:9000/sanitization/redaction/patterns" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X DELETE "http://localhost:9000/sanitization/redaction/patterns/custom_project_codes"
```
...
```bash
curl -X POST "http://localhost:9000/sanitization/pseudonym/generate" \
  -H "Content-Type: application/json" \
  -d '{
    "value": "Tim Cook",
    "dataType": "name",
...
```bash
curl -X POST "http://localhost:9000/sanitization/pseudonym/lookup" \
  -H "Content-Type: application/json" \
  -d '{
    "value": "Tim Cook",
    "dataType": "name"
...
```bash
curl -X POST "http://localhost:9000/sanitization/test" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X GET "http://localhost:9000/sanitization/stats"
```
**Response:**
```json
{
