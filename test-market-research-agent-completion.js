...
async function createMarketResearchTask() {
  console.log('\n📊 Creating market research task...');
  
  try {
    const taskRequest = {
      method: 'executeTask',
      prompt: 'Conduct a comprehensive market analysis for AI-powered business automation tools targeting mid-market companies (100-500 employees). Include market size, competitive landscape, key trends, and strategic recommendations for market entry.',
    ...
    const response = await fetch(`${API_BASE_URL}/agents/marketing/market_research/tasks`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
...
async function runTest() {
  console.log('🧪 Starting Market Research Agent Completion Test');
  ...
  // Step 2: Create market research task
  const taskData = await createMarketResearchTask();
  ...
  if (taskCompleted && hasResponse) {
    console.log('\n🎉 SUCCESS! Market Research agent is working correctly!');
    console.log('\n📊 Generated Market Analysis Preview:');
    console.log('=' .repeat(50));
